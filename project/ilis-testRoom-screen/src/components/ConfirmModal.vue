  ...
  <Teleport to="body">
    <div v-show="value" :class="`p-modal ${value ? 'show-modal' : ''}`">
      <div class="p-modal-wrap font-ys">
        <div class="p-modal-head">{{ title }}</div>
        <div class="p-modal-body tip-modal-body">
          <slot></slot>
        </div>
        <div class="p-modal-foot">
          <p v-if="!closeCancel" class="p-btn" @click="onClose">{{ cancelText }}</p>
          <p v-if="!closeOk" class="p-btn confirm" @click="okConfirm">{{ confirmText }}</p>
        </div>
      </div>
    </div>
  </Teleport>
