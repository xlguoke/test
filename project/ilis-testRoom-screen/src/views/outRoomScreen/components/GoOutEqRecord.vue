<template>
  <ComCard title="外出设备记录" is-long>
    <div class="task-list-wrap">
      ...
      <div id="outRecord" class="task-scroll-box">
        <ul :class="`task-list scroll-box ${moveTask ? 'startMove' : ''}`">
          <li v-for="d in data" :key="d.id" class="tr sp">
            ...
            <p :title="d?.projectName">{{ d?.projectName }}</p>
            <p :title="d?.dayCount">{{ d?.dayCount }}</p>
...
import { egressRecord } from "@/api/outRoom.api"
...
const getData = () => {
  egressRecord(props.labId).then((res) => {
