<template>
  <div class="task-list-wrap">
    ...
    <div id="checkExpireEq" class="task-scroll-box">
      <ul :class="`task-list scroll-box ${moveTask ? 'startMove' : ''}`">
        <li v-for="d in data" :key="d.id" class="tr sp">
          ...
          <p :title="formatDate(d?.nextCheckDate)">{{ formatDate(d?.nextCheckDate) }}</p>
        </li>
      </ul>
    </div>
  </div>
...
import { check } from "@/api/outRoom.api"
