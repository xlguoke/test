<template>
  <ComCard title="到期未归还设备统计" is-long>
    <div class="task-list-wrap">
      ...
      <div id="returnExpireEq" class="task-scroll-box">
        <ul :class="`task-list scroll-box ${moveTask ? 'startMove' : ''}`">
          <li v-for="d in data" :key="d.id" class="tr sp">
            ...
            <p :title="formatDate(d?.borrowTime)">{{ formatDate(d?.borrowTime) }}</p>
            <p :title="formatDate(d?.expectReturnTime)">{{ formatDate(d?.expectReturnTime) }}</p>
            <p :title="d?.borrowUser">{{ d?.borrowUser }}</p>
          </li>
        </ul>
      </div>
    </div>
...
import { egressOverdue } from "@/api/outRoom.api"
...
const getData = () => {
  egressOverdue(props.labId).then((res) => {
