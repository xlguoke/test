          ...
          >
            ...
            <van-row class="sample-info">
              ...
              <van-col span="12">
                ...
                {{ selTab == 1 ? d.inSaveSite : d.outSaveSite || "--" }}
              </van-col>
              <van-col span="12">
                <span class="label">存放地点：</span>
                {{ selTab == 1 ? d.inSaveSite : d.outSaveSite || "--" }}
              ...
              <van-col span="12">
                <span class="label">{{ selTab == 1 ? "入库" : "出库" }}人员：</span>
                {{ selTab == 1 ? d.inOperator : d.outOperator || "--" }}
              </van-col>
              <van-col span="12">
                <span class="label">{{ selTab == 1 ? "入库" : "出库" }}时间：</span>
                {{
                  selTab == 1
                    ? d.inTime
                      ? formatDate(d.inTime, 1)
                      : "--"
                    : d.outTime
                    ? formatDate(d.outTime, 1)
                    : "--"
                }}
              </van-col>
            </van-row>
            ...
            <img
              ...
              src="/src/assets/images/1223.png"
            />
          </li>
...
const getTestTaskData = async () => {
  loading.value = true
  let res = await api.getInventoryDetail(search.value)
  loading.value = false
  if (res.code === 20000) {
