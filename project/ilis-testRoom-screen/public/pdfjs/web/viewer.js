...
//var pixelRatio = devicePixelRatio / backingStoreRatio;	//原代码
  var pixelRatio = devicePixelRatio / backingStoreRatio * 2;//因为100%比例下显示不清楚,所以调整成这样,像素比例X2
  return {
    sx: pixelRatio,
    sy: pixelRatio,
    scaled: pixelRatio !== 1
    ...
    return loadingTask.promise.then(function (pdfDocument) {
      ...
      if (exception instanceof _pdfjsLib.InvalidPDFException) {
        ...
        for (var i = 0; i < vars.length; i++) {
          ...
          console.log("pai0----:"+ pair[0]);
          console.log("pai1----:"+ pair[1]);
          if (pair[0] == 'pdfStatus') {
            pdfStatus= pair[1];
          }
        }
       // console.log("参数参数:"+pdfStutas);
        if (pdfStatus == 'false'){
          loadingErrorMessage = _this3.l10n.get('invalid_file_converting', null, 'Invalid or corrupted PDF file.');
        } else {
