  ...
  <div>
    <a-spin tip="加载中..." :spinning="spinning">
      <a-row class="action_row">
        ...
        <div class="rt">
          <a-space>
            <a-button v-auth="'alarms::eq::analysis'" type="primary" @click="visibleAnalyse = true">
              立即分析
            </a-button>
          </a-space>
        </div>
...
import {
  getEquipmentWarnListAPI,
  getEquipmentAnalysisAPI,
...
const analyseSubmit = async () => {
  try {
    ...
    spinning.value = true
    let res = await getEquipmentAnalysisAPI(analyseForm.value)
    getEarlyWarnList()
...
let disposeType = ref("Analysis.EQ") //预警类型
...
const getEarlyWarnList = () => {
  ...
  getEquipmentWarnListAPI(queryFrom.value)
    .then((res) => {
