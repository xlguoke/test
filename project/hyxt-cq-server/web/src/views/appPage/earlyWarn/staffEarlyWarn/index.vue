...
let disposeType = ref("Analysis.Staff") //预警类型
...
const analyseSubmit = async () => {
  try {
    ...
    spinning.value = true
    let res = await getStaffAnalysisAPI(analyseForm.value)
    getEarlyWarnList()
...
const getEarlyWarnList = () => {
  spinning.value = true
  getStaffsWarnListAPI(queryFrom.value)
    .then((res) => {
