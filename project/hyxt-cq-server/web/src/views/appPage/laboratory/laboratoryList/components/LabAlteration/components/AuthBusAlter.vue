<template>
  <div class="">
    <a-spin tip="加载中..." :spinning="spinning">
      ...
      <a-row :gutter="10">
        <a-col :span="12">
          <p style="margin-bottom: 10px">变更前授权业务范围：</p>
          <a-table
            :data-source="paramsAlterBefore"
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'paramName'">
                <span v-if="record.isDel" style="color: red">{{ record.paramName }}</span>
              </template>
            </template>
          </a-table>
        </a-col>
        <a-col :span="12">
          <p style="margin-bottom: 10px">变更后授权业务范围：</p>
          <a-table
            :data-source="paramsAlterData"
            :columns="busColumns"
            bordered
            size="small"
            row-key="id"
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'paramName'">
                <span v-if="record.isAdd" style="color: green">{{ record.paramName }}</span>
              </template>
              ...
              <template v-if="column.key === 'handle'">
                <a-space>
                  ...
                  >
                    <a v-if="!props.details" size="small" type="primary" style="color: red" danger>
                      移除
                    </a>
                  </a-popconfirm>
                </a-space>
...
import {
  ...
  getPojectParmsAPI,
  addBusinessAlterAPI,
  deleteAlterBusinessAPI,
  updateAlterBusinessAPI
...
const labId: Ref<string> = toRefs(props).labId //试验室id
...
let paramsAlterBefore = ref([])
let paramsAlterData = ref([])
let beforeColumns = [
  {
    title: "试验项目",
    colSpan: 2,
...
const getLaboratoryQualify = async () => {
  let res = await getLaboratoryQualifyAPI(labId.value)
  qualifyList.value = []
  if (res) {
...
const updateBusiness = async (row) => {
  spinning.value = true
  let r = await updateAlterBusinessAPI(alterId.value, row)
  message.success("操作成功")
  getLabAuthBusinessList()
  spinning.value = false
}
const editAuthBusiness = (row: any) => {
  let decide = paramsAlterBefore.value.some((it: any) => {
    return it.isEdit
  })
  if (decide) {
...
const getLabAuthBusinessList = async () => {
  spinning.value = true
  let res: any = await getAlterBus(alterId.value)

  let deletedParamsId = res.removedBusiness.map((it) => {
    //被删除的参数id
    return it.paramId
  })
  let addParamsId = res.newBusiness.map((it) => {
    //新增的参数id
    return it.paramId
  })

  paramsAlterBefore.value = processData(res.preBusinessScopes).map((it) => {
    if (deletedParamsId.includes(it.paramId)) {
      it.isDel = true
    }
    return it
  })
  paramsAlterData.value = processData(res.businessScopes).map((it) => {
    if (addParamsId.includes(it.paramId)) {
      it.isAdd = true
    }
    return it
  })
...
const processData = (arr) => {
  ...
  myArr.forEach((it) => {
    it.data.forEach((cit) => {
      cit.parentItemRowSpan = 0 //清空每行跨行数
    })
    it.data[0].parentItemRowSpan = it.data.length //设置第一行跨行数
    newArr = [...newArr, ...it.data] //合并数据
  })
...
const removePojectParms = async (id) => {
  spinning.value = true
  let r = await deleteAlterBusinessAPI(alterId.value, id)
  spinning.value = false
  getLabAuthBusinessList()
}
...
const addAuthBusiness = async () => {
  ...
  try {
    await addBusinessAlterAPI(alterId.value, d)
  } catch (error) {
    spinning.value = false
  }
