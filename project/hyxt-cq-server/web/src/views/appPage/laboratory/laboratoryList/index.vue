  ...
  <div>
    <a-spin tip="加载中..." :spinning="spinning">
      <a-row class="action_row">
        ...
        <div class="rt">
          <a-space>
            <a-button v-auth="'laboratory::export'" @click="exportExcel">导出</a-button>
            <a-button v-auth="'laboratory::add'" type="primary" @click="openEditLaboratory('')">
              新增
            </a-button>
          </a-space>
        </div>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'handle'">
            <a-space>
              ...
              <a-button
                v-if="['运行中', '已到期'].includes(record.status)"
                v-auth="'laboratory::statusSwitchover'"
                size="small"
                type="primary"
                danger
              ...
              <a-button
                v-if="['变更未通过'].includes(record.status)"
                v-auth="'laboratory::revoke::cancel'"
                size="small"
                type="primary"
                danger
...
const exportExcel = () => {
  ...
  exportExcelAPI(q)
    .then((data: any) => {
      ...
      elink.setAttribute("download", "工地试验室Excel导出.xlsx")
