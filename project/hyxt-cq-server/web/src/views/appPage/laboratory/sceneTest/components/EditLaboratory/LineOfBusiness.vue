            ...
            <div class="rt">
              <a-space>
                ...
                <a-button
                  v-if="!props.details"
                  type="primary"
                  @click="downloadByNameOrUrl('现场检测项目业务范围数据导入模板.xlsx')"
                >
                  下载模板
                </a-button>
              </a-space>
...
import {
  addBusinessScopeAPI,
  updateBusinessScopeAPI,
  getBusinessScopeAPI,
  deleteBusinessScopeAPI,
  getBusinessScopeDetPI,
  importBusinessScopeAPI
...
const editBusiness = async (row) => {
  ...
  await clearFrom()
  let res = await getBusinessScopeDetPI(row.id, row.labId)
  submitForm.value = res
  spinning.value = false
}

const customRequest = async (op) => {
  if (op.file.type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
    message.warning("请上传.xlsx格式文件")
    return
  }
  spinning.value = true
  ...
  try {
    let res = await importBusinessScopeAPI(formData)
    spinning.value = false
    if (res.length) {
      res.forEach((it: any) => {
        message.warning(it)
...
const handleBusinessOk = async () => {
  ...
  try {
    submitForm.value.id
      ? await updateBusinessScopeAPI(submitForm.value)
      : await addBusinessScopeAPI(submitForm.value)
...
const getLineOfBusinessList = async () => {
  spinning.value = true
  let res = await getBusinessScopeAPI(queryFrom.value)
...
const removeBusiness = async (row: any) => {
  await deleteBusinessScopeAPI(row.id, row.labId)
  getLineOfBusinessList()
}
