<template>
  ...
  >
        ...
        <a-col :span="12">
          ...
          >
            <a-select
              ...
              show-search
              :disabled="userInfo.type == 'PCM' || isDetails"
              placeholder="请选择建设单位！"
            ...
            <common-select
              ...
              @change="
                (v) => {
                  ...
                  record.businessLicenseRegistrationNumber =
                    v.option.businessLicenseRegistrationNumber
                }
              "
            />
          </template>
    ...
    <a-select
      ...
      :disabled="isDetails || projectFrom.id"
...
const openModel = (id, isDet = false) => {
  ...
  if (id) {
    ...
    fromTitle.value = "新增"
    if (userInfo.value.type == "PCM") {
      nextTick(() => {
        projectFrom.value.constructorId = userInfo.value.orgId
        projectFrom.value.constructor = userInfo.value.orgName
      })
    }
  }
