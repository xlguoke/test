import { getMenuForUserIdAPI } from "@/api/system.api"
...
export const previewPDF = (url: string) => {
  ...
  console.log(geturl(`/static/pdfjs-es5/web/viewer.html?file=${url}`))
  const isSygcjc = location.href.indexOf("sygcjc") !== -1
  let page: any = window.open(
    `${isSygcjc ? "/sygcjc" : ""}/static/pdfjs-es5/web/viewer.html?file=${url}`,
    "文件预览"
  )
  if (!page) return
  page.onload = () => {
...
//获取最新菜单数据及权限
export const getMenuData = async (): Promise<any> => {
  let res = await getMenuForUserIdAPI()
  //处理子级菜单继承父级的权限
  res.forEach((item: any) => {
    if (item.children && item.children.length) {
      let childrenArr = item.children
      childrenArr.forEach((cl: any) => {
        cl.auths = [...(cl.auths || []), ...(item.auths || [])]
      })
    }
  })
