...
import { debounce, deepCopy } from "@/assets/js/utils"
import minioPutObject, { getMinioFileUrl } from "@/config/minio.config.js"
...
const props = defineProps({
  ...
  hideDelete: Boolean,
  beforeUpload: Function // 返回 false 或 Promise.reject 时拦截上传
})
...
const uploadFun = async (fileList) => {
  ...
  for (let i = 0; i < fileList.length; i++) {
    ...
    try {
      // 上传
      const { url, etag } = await minioPutObject(file, bucketUri)
      let _file = {
        uid: file.uid,
        name: file.name,
        status: "done",
