...
import type { ICustomRow, IDeleteConfig, ITableHookConfig } from '~/interface/ITableHookConfig'
...
export interface ICustomColumns {
  ...
  width?: number
  ellipsis?: boolean
  /** 是否按领域配置替换列名称，为false时不替换，为true或undefined时替换 */
  replaceTermName?: boolean
...
export function useTableHooks<T extends { id: string | number, disabled?: boolean }>(config: ITableHookConfig<T>) {
   ...
   * # 表格高度hooks
   */
  const { tableBoxRef, tableHeight, initTableHeight } = useTableHeight(isPagination, dataSource, loading)
    ...
    if (config.delApi) {
      ...
      Modal.confirm({
        ...
        async onOk() {
          ...
          if (dataSource.value.length === 1 && page.value > initialPage) {
            page.value--
          }
          getList()
        },
  ...
  function getPagination(mixinConfig?: Omit<TablePaginationConfig, 'total' | 'pageSize' | 'current' | 'onChange'>) {
    ...
    return {
      total: total.value,
      pageSize: size.value,
      current: initialPage === 1 ? page.value : page.value + 1,
    ...
    return {
      ...
      onSelect: (record: T, selected: boolean) => {
        if (selected) {
          ...
          const ind = selectedRows.value.findIndex(d => d.id === record.id)
          selectedRowKeys.value.splice(ind, 1)
          selectedRows.value.splice(ind, 1)
        }
      },
      onSelectAll: (selected: boolean, _selectedRows: T[], changeRows: T[]) => {
        if (selected) {
          selectedRowKeys.value = [...selectedRowKeys.value, ...changeRows.map(d => d.id)]
          selectedRows.value = [...selectedRows.value, ...changeRows]
        }
        else {
          selectedRowKeys.value = selectedRowKeys.value.filter(d => !changeRows.map(d => d.id).includes(d))
          selectedRows.value = selectedRows.value.filter(d => !changeRows.map(i => i.id).includes(d.id))
        }
      },
      ...mixinConfig,
    }
