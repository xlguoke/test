...
<template>
  <a-form-item
    label="审核人员"
    :label-col="{ span: labelCol || 5 }"
    :wrapper-col="{ span: 15 }"
  >
    <div>
      <span v-for="(item, index) in personData">
        <a-tag
          v-if="(Array.isArray(item) && item?.length === 1) || !Array.isArray(item)"
          @close="preventDefault(item?.[0].activitiNodeId)"
          @click="setPersonnels('radio', item?.[0].activitiNodeId, '请选择人员')"
        >
          {{ item.presetUserRealName || item.processTaskName }}
        </a-tag>
        <div v-else-if="Array.isArray(item) && item?.length > 1" class="border border-solid border-gray-300 rounded py-1 px-2 inline-block">
          <a-tag
            v-for="tag in item"
            :key="tag.presetUserRealName"
            @close="preventDefault(tag.activitiNodeId)"
            @click="setPersonnels('radio', tag.activitiNodeId, '请选择人员')"
          >
            {{ tag.presetUserRealName || tag.processTaskName }}
          </a-tag>
        </div>
...
export default {
  ...
  props: ['callback', 'labelCol', 'readonly'],
  data() {
    return {
      personData: [
        {
