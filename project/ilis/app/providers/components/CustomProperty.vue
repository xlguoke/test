  ...
  <div>
    <div>
      <div class="mb-2">
        ...
        <div class="my-2 pt-2" style="border-top: solid 1px var(--colorSplit);">
          <a-button @click="add">
            新增
          </a-button>
          <a-button @click="edit">
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.dataIndex === 'visible'">
            <span>
              <a-tag :color="record.visible ? 'green' : 'volcano'">
                {{ record.visible ? '可见' : '不可见' }}
              </a-tag>
            </span>
          </template>
          <template v-if="column.dataIndex === 'columnType'">
            <span>{{ names[record.columnType] }}</span>
          </template>
        </template>
      </a-table>
      <ht-modal v-model:open="addColumnVisible" title="自定义属性" @cancel="handleCancel">
        <a-form
          v-bind="layout"
          ref="columnForm"
          :model="columnForm"
          :rules="rules"
        >
          <a-form-item has-feedback label="属性名称" prop="columnName">
            <a-input v-model:value="columnForm.columnName" autocomplete="off" />
          </a-form-item>
          <a-form-item has-feedback label="属性类型" prop="columnType">
            <a-select
              v-model:value="columnForm.columnType"
              placeholder="请选择属性类型"
            >
          ...
          <a-form-item
            v-if="columnForm.columnType === 'select'"
            has-feedback
            label="属性候选值"
            prop="columnValue"
          >
            <a-popover title="提示">
              ...
              <a-select
                v-model:value="columnForm.columnValue"
                :allow-clear="true"
                :token-separators="[',']"
                mode="tags"
          ...
          <a-form-item has-feedback label="排序号" prop="columnIndex">
            <a-input-number v-model:value="columnForm.columnIndex" />
          </a-form-item>
          <!-- <a-form-item has-feedback label="是否可见" prop="visible"> -->
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { title: '属性名称', dataIndex: 'columnName' },
        {
          title: '属性类型',
  ...
  methods: {
    ...
    handleOk() {
      this.$refs.columnForm.validateFields().then(() => {
        this.loading = true
