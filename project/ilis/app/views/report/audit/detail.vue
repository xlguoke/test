  ...
  >
    <div class="w-full h-full flex flex-col">
      ...
      <a-alert
        ...
        message="系统要求审核阶段盲样，但本报告未生成盲样pdf，故无盲样文件可展示。（tips: 如需展示，请退回检测，并重新提交以生成盲样PDF。）"
        type="warning"
        show-icon
      />
      <div class="w-full flex-1 h-0 flex overflow-hidden p-[12px] ">
        <div class="relative  shrink-0 h-full mr-[12px] bg-[var(--colorBgContainer)] ">
          <div
            class="relative h-full w-[300px] transition-all overflow-hidden"
          ...
          >
            <VueDraggable
              ...
              :animation="300"
              handle=".handle"
              class="absolute inset-0 w-[300px] overflow-y-auto transition-all"
          ...
          >
            <template #action-after="{ direction }">
              <div class="bg-[var(--colorBgContainer)] p-[8px] flex" :class="[direction === 'vertical' ? 'w-full flex-col mt-[14px]' : 'h-full flex-row ml-[16px]']">
                <div
                  v-if="type !== '1' && type !== '2'"
                  v-permission="'reportAuditRollback'"
                ...
                <div
                  v-if="type !== '1' && type !== '2'"
                  v-permission="'doReportAuditPass'"
    ...
    <ht-modal
      ...
      hide-confirm
      fixed-height
      @cancel="projectDetailVisible = false"
    >
      <iframe
        style="border: 0"
        width="100%"
        height="100%"
        :src="projectDetailUrl"
      ></iframe>
    </ht-modal>
  </IlisContainer>
</template>
...
import { getComprehensiveTestScheme, getOfflineReportFile, getReportConvertStatus, isOfflineReportFun, ReportTypeDict } from '../approve/api'
import DetailHeader from '../approve/components/DetailHeader.vue'
import { getAuditReportRelatedInfoDetail, getReportFile } from './api'
...
const actionItems = ref<IActionItem[]>([
  ...
  },
  {
    ...
    isShow: () => String(type) !== '1' && String(type) !== '2' && !reportType.includes('synthesis'),
  },
  {
    label: '数据中心',
  ...
  },
  {
    ...
    onClick: recordTrace,
    icon: () => h('img', { src: DetailBtnImg }),
    isShow: () => String(type) !== '1' && String(type) !== '2' && !reportType.includes('synthesis') && hasPermission('operationalRecords'),
  },
])
...
async function getReportAttachments() {
  ...
  if (data?.obj) {
    ...
    reportFileObj.forEach((item: any) => {
      ...
      let subTitle = ''
      let url = ''
      let icon = () => h('i', { class: 'iconfont icon-lujing-2' })
      // 根据类型设置标题和URL
      switch (item.type) {
        case '1': // 报告
        case '3': // 记录
...
async function addDetailViewToFileItems() {
  ...
  if (reportType === 'synthesis' || reportType === 'synthesis_temp' || reportType === 'synthesis_Temp') {
    ...
    detailUrl = `${import.meta.env.VITE_ILIS_BASE}/reportCreateController.do?goEditPage&detectionDetail=detectionDetail&reportId=${reportId}&detail=detailPage&reportPage=1`
  }
...
async function getDetailInfo() {
  const { data } = await getAuditReportRelatedInfoDetail(reportId)
...
/** 工程项目链接点击事件 */
function handleProjectLinkClick(consignProjectId: string) {
  if (!consignProjectId) {
    console.warn('工程项目ID不存在')
    return
  }
  // 打开工程项目详情弹窗
  projectDetailUrl.value = `itemController.do?goTab&id=${consignProjectId}&status=1`
  ...
  const consignInfo = {
    ...
    options: [
      { label: '委托单位', value: consign?.consignUnitName || '' },
      {
        label: '工程项目',
        value: () => h('span', {}, [
          consign?.unitProjectName || '',
          consign?.consignProjectId
            ? h('a', {
                class: 'ml-2 text-colorPrimary cursor-pointer',
                onClick: () => handleProjectLinkClick(consign.consignProjectId),
              }, '查看详情>>>')
            : null,
        ]),
      },
