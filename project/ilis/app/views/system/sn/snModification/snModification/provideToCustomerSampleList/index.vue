<template>
  <div class="testReport">
          ...
          <div v-if="allSearch">
            <a-form style="width: 520px" :model="formState">
              <a-form-item
                ...
                :wrapper-col="{ span: 17 }"
              >
                <a-input v-model:value="formState.provideToCustomerSampleCode" />
              </a-form-item>
              <a-form-item
                label="样品编号"
                :label-col="{ span: 5 }"
    ...
    <ProvideToCustomerSampleEdit
      ref="provideToCustomerSampleEdit"
      :callback="getData"
    />
  </div>
</template>
...
import ProvideToCustomerSampleEdit from './components/provideToCustomerSampleEdit.vue'
...
export default {
  ...
  methods: {
    ...
    editCode(e, record) {
      e.stopPropagation()
      e.preventDefault()
      this.$refs.provideToCustomerSampleEdit.showModal(record)
    },
    getData(flag) {
      let data = {}
    ...
    sorterChange(pagination, filters, sorter) {
      if (sorter) {
        ...
        if (sorter.field === 'provideToCustomerSampleCode') {
          this.sortParams = { provideToCustomerSample: sorterObj[sorter.order] }
        }
        // 样品编号
