          ...
          <div v-if="allSearch">
            <a-form style="width: 520px" :model="formState">
              <a-form-item
                ...
                :wrapper-col="{ span: 17 }"
              >
                <a-input v-model:value="formState.testRecordCode" />
              </a-form-item>
              <a-form-item
                label="编号生成时间"
                :label-col="{ span: 5 }"
                :wrapper-col="{ span: 17 }"
              >
                <a-range-picker v-model:value="formState.createDates" />
              </a-form-item>
              <a-form-item
                label="任务分配日期"
                :label-col="{ span: 5 }"
                :wrapper-col="{ span: 17 }"
              >
                <a-range-picker v-model:value="formState.taskAssignDates" value-format="YYYY-MM-DD" />
              </a-form-item>
              <a-form-item
                label="编号格式类型"
                :label-col="{ span: 5 }"
      ...
      <a-form>
        ...
        >
          <span class="form-item-span">
            {{ editData ? editData.testRecordCode : '' }}
          </span>
        </a-form-item>
        <a-form-item
          label="新记录编号"
  ...
  methods: {
    ...
    getData(flag) {
      ...
      if (this.allSearch) {
        ...
        const taskAssignDatesValue = fieldsValue.taskAssignDates
        if (taskAssignDatesValue && taskAssignDatesValue.length > 0) {
          fieldsValue.taskAssignDates = `${taskAssignDatesValue[0]} ~ ${taskAssignDatesValue[1]}`
        }
        ...
        data = {
          ...fieldsValue,
          taskAssignDates: fieldsValue.taskAssignDates
            ? fieldsValue.taskAssignDates
            : '',
    ...
    sorterChange(pagination, filters, sorter) {
      if (sorter) {
        ...
        // 任务分配日期
        if (sorter.field === 'taskAssignDate') {
          this.sortParams = { taskAssignDateOrder: sorterObj[sorter.order] }
        }
        // 样品编号
        if (sorter.field === 'testObjectCode') {
          this.sortParams = { testObjectCodeOrder: sorterObj[sorter.order] }
        }
        // 委托编号
