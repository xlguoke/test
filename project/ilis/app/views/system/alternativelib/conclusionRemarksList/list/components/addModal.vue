...
export default {
  props: ['isAdd', 'callback', 'formType', 'sampleId'],
  data() {
    return {
      visible: false,
  ...
  methods: {
    ...
    handleOk() {
      this.$refs.formRef.validateFields().then(() => {
        ...
        const pageType = getQueryVariable('pageType') // 1  ： 层级备选库
        let data = {
          description: '',
          ...this.data,
        ...
        if (Number.parseFloat(data.sort) % 1 != 0) {
          message.error('请输入整数序号！')
          return
        }
        if (this.sampleId) {
          data.type = 'special'
          data.testSampleId = this.sampleId
        }
        else {
          data.type = 'universal'
        }
