  ...
  <a-collapse v-model:active-key="activeKey">
    <a-collapse-panel key="1" :header="sampleInfo.testObjectCode">
      <a-form
        ...
        :disabled="disabled"
      >
        <a-form-item v-if="enableField.testSampleDisplayName" :label="term('样品名称')">
          <a-input v-model:value="form.testSampleDisplayName" :placeholder="`请输入${term('样品名称')}`" />
        </a-form-item>
        <a-form-item v-if="enableField.standard" :label="term('规格型号')">
          <a-input-group compact class="!flex" @click="emits('setSpecification')">
            <a-input v-model:value="form.standard" readonly :placeholder="`请输入${term('规格型号')}`" />
            <a-button :disabled="false">
              <SettingOutlined />
            </a-button>
          </a-input-group>
        </a-form-item>
        <a-form-item v-if="enableField.testPart" :label="term('检测部位')">
          <a-input-group compact class="!flex" @click="showTestParts">
            <a-input
              v-model:value="form.testPart"
              icon="md-filing"
              readonly
              :placeholder="`请输入${term('检测部位')}`"
            />
            <a-button>
              <SettingOutlined />
            </a-button>
        ...
        <a-form-item v-if="enableField.projectPartAndUse" :label="term('工程部位/用途')">
          <a-input v-model:value.trim="form.projectPartAndUse" :placeholder="`请输入${term('工程部位/用途')}`" />
        </a-form-item>
        <a-form-item :label="`${term('样品')}描述`">
          <a-auto-complete v-model:value.trim="form.description" :options="descriptionOptions" :placeholder="`请输入${term('样品')}描述`" />
        </a-form-item>
        <a-form-item v-if="enableField.sampleNum" :label="`${term('样品数量')}`">
          <a-input-number
            v-model:value="sampleNum"
            :min="0"
            :placeholder="`请输入${term('样品数量')}`"
            class="w-full"
          >
            <template #addonAfter>
              <a-select
        ...
        <a-form-item v-if="enableField.delegatesNum" label="代表数量">
          <a-input-number
...
const enableField = reactive({
  /** 样品名称 */
  testSampleDisplayName: !!(getField('testSampleDisplayName')?.selected),
  /** 检测部位 */
  testPart: !!(getField('testPart')?.selected),
  /** 规格型号 */
  standard: !!(getField('standard')?.selected),
  /** 工程部位/用途 */
  projectPartAndUse: !!(getField('projectPartAndUse')?.selected),
  /** 样品数量 */
  sampleNum: !!(getField('sampleNum')?.selected),
  /** 代表数量 */
  delegatesNum: !!(getField('delegatesNum')?.selected),
})
...
function showTestParts() {
  ...
  if (vals.length < group) {
    testParts.value = Array.from({ length: group - vals.length }).fill('')
  }
