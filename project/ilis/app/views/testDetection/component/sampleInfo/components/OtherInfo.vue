...
function initOtherInfo() {
  ...
  list.forEach((item: LocalSpecifications) => {
    // 兼容处理
...
function initInputHistory(item: Specifications) {
  ...
  if (item.listValue) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (ind === -1) {
        newHistory.push({
          label: UseTestOtherInfo.getListValueLabel(list[i]),
          value: UseTestOtherInfo.getListValueValue(list[i]),
          isDataCenter: !item.customized || list[i].includes('@_@'),
        })
      }
      else {
