...
import type { ViewSampleTreeNode } from './UseTestSample.ts'
import type {
  BuildParamsOpts,
  ParamListFilterObj,
  TestParamsItem,
...
export class UseSelectTestParam extends BaseTestParams {
  ...
  // 当前选中的样品树节点
  sampleNode: ViewSampleTreeNode | undefined
  ...
  /** 更新选择的样品数据 */
  updateSampleNode(node: ViewSampleTreeNode) {
    this.sampleNode = node
  }

  /** 是否启用样品单价 */
  get enableSamplePrice() {
    const price = this.sampleNode?.attributes?.samplePrice
    return price !== null && price !== undefined && price !== ''
  }

  /** 样品单价 */
  get samplePrice() {
    const price = this.sampleNode?.attributes?.samplePrice || 0
    return Number(price)
  }
  ...
  get totalTestQuantity() {
    ...
    this.selectedRows.forEach((item) => {
      ...
      if (item.shareTestQuantityIdent) {
        ...
        sum += item.sampleTestQuantity || item.testQuantity || 0
      }
    })
  ...
  get priceDescription() {
    ...
    if (this.enableSamplePrice)
      return selectList.length ? [`${this.term('样品')}单价：${this.samplePrice}元`] : []
  ...
  init() {
    getBusinessParams(['SELECTED_SAMPLE_LAST_LAYER', 'DISPLAYS_TEST_PERIOD', 'DISPLAYS_TEST_QUANTITY']).then((res) => {
      this.SELECTED_SAMPLE_LAST_LAYER = !!res.SELECTED_SAMPLE_LAST_LAYER
      this.DISPLAYS_TEST_PERIOD = !!res.DISPLAYS_TEST_PERIOD
      this.DISPLAYS_TEST_QUANTITY = !!res.DISPLAYS_TEST_QUANTITY
    })
  }
  ...
  buildParamsData(opts: BuildParamsOpts): ViewTestParamsItem[] {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (isInit) {
        ...
        item.selectedTestQuantity = item.sampleTestQuantity || item.testQuantity
   ...
   */
  localFilterData(filterObj: ParamListFilterObj, useBasicInfo: UseBasicInfo) {
    this.data = this.filterData(this.cacheData, filterObj, useBasicInfo)
  }
