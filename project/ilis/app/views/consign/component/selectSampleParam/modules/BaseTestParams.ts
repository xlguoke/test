import type { UseBasicInfo } from '../../selectSample/modules/UseBasicInfo.ts'
...
export class BaseTestParams extends UseModuleEmit<{ testParams: any }> {
  ...
  constructor(sourceModule: SourceModuleType, consginPageParam?: IConsginPageParam) {
    super()

    this.sourceModule = sourceModule
    this.consginPageParam = consginPageParam
  }
   ...
   */
  buildParamsData(opts: BuildParamsOpts, useBasicInfo: UseBasicInfo): ViewTestParamsItem[] {
    const data = opts.data
    const isInit = opts.isInit
    const queryParam = opts.queryParam
    const enableSamplePrice = useBasicInfo.enableSamplePrice
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      // 原价格项
      const customPriceItem = prices.find(j => j.type === PRICE_TYPE.CUSTOM)
      ...
      if (this.sourceModule === 'UseTestParams') {
        // 是否同时存在原价格和共用价格
        if (sharePriceItem && customPriceItem && item.selectedPriceType === PRICE_TYPE.CUSTOM) {
          // 详情模式下，若有存在之前的价格被打包为了共用单价，则按照之前的原价格渲染，不渲染进共用价格中
          if (this.consginPageParam && this.consginPageParam.pageState.isDetailPage) {
      ...
      // 共用价格数据关系构建
      if (!enableSamplePrice && sharePriceItem && sharePriceItem.qualifier) {
  ...
  renderSharePriceRow(item: SharePriceGroupObjItem, shareQualifier: string) {
    ...
    tipRow.displayName = `以下${this.term('试验参数')}共用单价（￥${item.price}） - ${shareQualifier}`
  ...
  renderNotSharePriceRow() {
    const tipRow = new ViewTestParamsItem()
    tipRow.displayName = `以下${this.term('试验参数')}非共用单价${this.term('试验参数')}`
  ...
  setDefaultPrice(item: ViewTestParamsItem) {
    ...
    if (!isDefined(item.priceType) || !item.selectedPriceId) {
      // 默认取排在第一个的价格
      let priceItem = prices[0]

      // 编辑界面，原价格特殊逻辑
      // 若之前选中价格被删除，后续又被设置在了共用价格中，则按共用价格展示
      if (this.sourceModule === 'UseTestParams' && this.consginPageParam?.pageState.isEditPage && priceItem.type === PRICE_TYPE.CUSTOM) {
        const sharePriceItem = prices.find(i => i.type === PRICE_TYPE.SHARE)
        if (sharePriceItem) {
          priceItem = sharePriceItem
        }
      }
   ...
   */
  filterData(testParams: ViewTestParamsItem[] | TestParamsItem[], filterObj: ParamListFilterObj, useBasicInfo: UseBasicInfo) {
