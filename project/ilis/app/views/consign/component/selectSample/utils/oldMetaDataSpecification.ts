...
import { connectorValue, DEFAULT_VALUE, SYSTEM_FIELDS_ENUM, SYSTEM_FIELDS_NAME } from '../../selectSpecification'

/**
 * 还原规格型号明细后，按拼接值排序
 * @param {TestOtherInfoMetaDataItem[]} specInfo 收样辅助信息元数据
 * @param {MetaDataEntity} metaData 元数据
 */
function sortSpecificationByStandard(specInfo: TestOtherInfoMetaDataItem[], metaData: MetaDataEntity): TestOtherInfoMetaDataItem[] {
  ...
  SYSTEM_FIELDS_NAME.forEach((item) => {
    const field = SYSTEM_FIELDS_ENUM[item]
...
export function compatibilityOldMetaData(metaData: MetaDataEntity): TestOtherInfoMetaDataItem[] {
  let specInfo = metaData.otherInfos.filter((item) => {
    const bol = SYSTEM_FIELDS_NAME.includes(item.systemFieldName)
