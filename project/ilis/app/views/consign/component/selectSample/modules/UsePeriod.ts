...
export class UsePeriod extends UseBase<PeriodEntity> {
  ...
  /** 计算预计试验日期 */
  calcPlannedTestDate(formingDate?: string | Dayjs, days?: number) {
    let day = days ? Number.parseFloat(`${days}`) : 0
    if (!formingDate || !day || Number.isNaN(day)) {
      return ''
    }
    day = day * 24 * 60 * 60 * 1000
    const date = dayjs(formingDate).valueOf()
    const { value } = useDateFormat(date + day, 'YYYY-MM-DD')
    return value
  }
}
