...
import { ParamPriceItem, PRICE_TYPE } from '~/views/consign/component/selectSampleParam/modules/UseParamPrice.ts'
...
export class UseTestParams extends BaseTestParams {
  ...
  constructor(private useBasicInfo: UseBasicInfo, consginPageParam: IConsginPageParam) {
    super('UseTestParams', consginPageParam)
  ...
  calcScriptPrice(useSampleInfo: UseSampleInfo, useTestOtherInfo: UseTestOtherInfo) {
    ...
    const calculateWithScript = this.consginPageParam.feeCalculateWithScript
  ...
  /** 根据规程名称和颁布号生成规程的唯一key */
  uniqueKey(standardName: string) {
    return standardName.replace(/[\s/《》()（）<>.。、:：〔〕+~“”，,\\;；·【】[\]—-]/g, '').toLowerCase()
  }
  ...
  formatTestParams2View(testParams: TestParamsMetaDataItem[]) {
    ...
    for (let i = 0; i < testParams.length; i++) {
      ...
      metaDataItem.criterion.forEach((j) => {
        const type = `${j.baseStandardUseType}` as STANDARD_TYPE
        if (!j.uniqKey) {
          j.uniqKey = this.uniqueKey(j.baseStandardName + (j.publishCode || ''))
        }
        if ([STANDARD_TYPE.BASIS, STANDARD_TYPE.ALL].includes(type)) {
          j.selected = true
          viewItem.basis.push(j)
        }
        if ([STANDARD_TYPE.STANDARD, STANDARD_TYPE.ALL].includes(type)) {
          j.selected = true
  ...
  setData(metaData: MetaDataEntity) {
    ...
    this.initTestParamList({
      ...
      priceStandardId: this.consginPageParam.priceStandardId,
