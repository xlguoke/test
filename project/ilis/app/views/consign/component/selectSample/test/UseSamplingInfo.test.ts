...
const consginPageParam: IConsginPageParam = {
  title: 'Test title',
  industryName: 'Test industryName',
  industryId: 'Test industryId',
  pageState: {} as any,
  consignDetail: {} as any,
  sampleDatas: [],
  SYSTEM_PARAM: {} as any,
  placeholder: vi.fn(),
  feeCalculateWithScript: vi.fn(),
  onSaveSample: vi.fn(),
}
...
describe('useSamplingInfo data and view', () => {
  ...
  // init class
  const useSamplingInfo = new UseSamplingInfo(consginPageParam)
  ...
  it('should bind data correctly', async () => {
    ...
    expect((wrapper.find('#form_item_samplingPerson').element as HTMLInputElement).value).toBe('Test samplingPerson')
    expect((wrapper.find('#form_item_samplingCompany').element as HTMLInputElement).value).toBe('Test samplingCompany')
    expect((wrapper.find('#form_item_samplingPersonNumber').element as HTMLInputElement).value).toBe('Test samplingPersonNumber')
    expect((wrapper.find('#form_item_samplingPlace').element as HTMLInputElement).value).toBe('Test samplingPlace')
