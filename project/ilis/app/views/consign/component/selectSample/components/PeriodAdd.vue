  ...
  >
    ...
    >
      <template v-if="!isEdit">
        <a-form-item :label="`${term('样品')}组别编号`" name="objectGroupNum" required>
      ...
      <template v-if="!isEdit">
        <a-form-item label="试件编号" name="periodCode">
          <a-input v-model:value="formData.periodCode" placeholder="请输入试件编号" />
        </a-form-item>
        <a-form-item label="制样日期" name="formingDate">
        ...
        <a-form-item label="预计试验日期" name="plannedTestDate">
          <a-input v-model:value="formData.plannedTestDate" disabled placeholder="填写制样日期和龄期后自动计算" />
        </a-form-item>
...
import type { UsePeriod } from '../modules/UsePeriod.ts'
...
watch(
  () => [formData.value.formingDate, formData.value.days],
  ([formingDate, days]: any) => {
    formData.value.plannedTestDate = usePeriod.calcPlannedTestDate(formingDate, days)
  },
)

const rules: Record<string, Rule[]> = {
  objectGroupNum: [{ required: true, message: `请选择${term('样品')}组别编号`, trigger: 'change' }],
...
async function handleOk() {
  formRef.value.validate().then(async () => {
    ...
    const testParamId: string[] = []
    const testParamName: string[] = []

    if (data.testParamId_TEMP) {
