      ...
      <div class="flex-1 w-0 flex flex-col gap-2">
        ...
        <div class="flex h-0">
          <a-table
            ...
            :pagination="false"
            bordered
            :scroll="{ y: 'calc(50vh - 76px)' }"
          >
            <template #bodyCell="{ column, record, text }">
              <template v-if="column.dataIndex === 'baseStandardUseType1'">
                <a-checkbox :checked="getCheckState(record as ParamStandardItem, STANDARD_TYPE.BASIS)" @click="onSelectStandard(record.uniqKey, STANDARD_TYPE.BASIS)" />
              </template>
              <template v-else-if="column.dataIndex === 'baseStandardUseType2'">
                <a-checkbox :checked="getCheckState(record as ParamStandardItem, STANDARD_TYPE.STANDARD)" @click="onSelectStandard(record.uniqKey, STANDARD_TYPE.STANDARD)" />
              </template>
              <template v-else-if="column.dataIndex === 'clauseCode'">
                <a-input v-model:value="record.clauseCode" placeholder="请输入" />
              </template>
              <template v-else-if="column.dataIndex === 'executeDate'">
                <span :style="isExpire(record.expireDate) ? { color: '#ff5722' } : {}">
                  {{ record.executeDate ? dayjs(record.executeDate).format('YYYY-MM-DD') : "" }}
                </span>
              </template>
              <template v-else-if="column.dataIndex === 'Action'">
                <a-space>
                  <ReadOutlined style="font-size: 16px;color:#1677ff;" title="查看详情" @click="onCheckDetail(record as ParamStandardItem)" />
                  <a-popover v-model:open="record._popover" title="将当前规程应用到以下参数" trigger="click">
                    <template #content>
                      <a-space direction="vertical">
                        ...
                        <a-space>
                          <a-button size="small" type="primary" @click="add2OtherParam(record as ParamStandardItem)">
                            确定
                          </a-button>
                    ...
                    <CopyOutlined style="font-size: 16px;color:#1677ff;" title="应用到其他参数" @click="onUse2OtherParam(record)" />
                  </a-popover>
                  <DeleteOutlined style="font-size: 16px;color:#f00;" title="取消引用" @click="onDel(record as ParamStandardItem)" />
                </a-space>
              </template>
              <template v-else>
                <span :style="isExpire(record.expireDate) ? { color: '#ff5722' } : {}">
...
const columns: ColumnsType = [
  { title: '检测依据', dataIndex: 'baseStandardUseType1', align: 'center' },
  { title: '评定标准', dataIndex: 'baseStandardUseType2', align: 'center' },
  ...
  { title: '操作', dataIndex: 'Action', width: '90px', align: 'center' },
]
...
// 过期
function isExpire(expireDate?: number) {
...
function onSelectStandard(uniqKey: string, type: StandardType) {
  const item = standardList.value.find(i => i.uniqKey === uniqKey) as ParamStandardItem
...
// 应用到其他参数
function onUse2OtherParam(item: any) {
...
function add2OtherParam(item: ParamStandardItem) {
  const checkedList = checkState.checkedList as string[]
  ...
  for (const key in dataSource.value) {
    if (key !== selectedTestParamId.value && checkedList.includes(key)) {
      ...
      // 存在相同规程，做覆盖处理
      const sameItem = standards.find(i => i.uniqKey === item.uniqKey) as ParamStandardItem
      if (sameItem) {
...
async function quoteNewStandards() {
  ...
  const data = res.data
  data.forEach((i: any) => {
    const item = i.standard || {}
    if (item.uniqKey && !standardList.value.find(j => j.uniqKey === item.uniqKey)) {
