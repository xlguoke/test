<template>
  <span class="attachment-handler">
    ...
    <!-- 只读模式下显示文件列表 -->
    <span v-if="readonly && !picViewMode && fileList.length > 0" class="readonly-file-list">
      <a
...
export default {
  ...
  methods: {
    ...
    async initAttachmentHandler() {
      ...
      try {
        const qrUrl = this.fixedQrUrl || await this.getQrURL(this.businessId, this.businessType)
        this.qrKey = this.getQrKey(qrUrl)
        this.$emit('init', { qrKey: this.qrKey, qrUrl })
    ...
    async getQrURL(id, type) {
      ...
      if (code !== 20000) {
        throw new Error(data.msg || data.message || '获取二维码URL失败')
      }
    ...
    getQrKey(qrUrl) {
      const newQrUrl = new URL(qrUrl)
      const qrKey = newQrUrl.searchParams.get('key')
      return qrKey
    },
    ...
    async getHistoryFile(qrKey) {
      ...
      const [{ code: fileCode, data: fileData }, { code: historyCode, data: historyFileData }] = await Promise.all([
        mRequest.get(api.common.getAttachmentByKey(qrKey)),
        this.businessId
          ? mRequest.get(`/rest/attachmentQR/historical/attachments/${this.businessType}`, {
              params: {
                businessId: this.businessId,
              },
            })
          : Promise.resolve({ code: 20000, data: [] }),
      ])

      if (fileCode !== 20000) {
        throw new Error(fileData.msg || fileData.message || '获取历史附件失败')
      }

      if (historyCode !== 20000) {
        throw new Error(historyFileData.msg || historyFileData.message || '获取历史附件失败')
      }
            ...
            if (file.attachmentId) {
              try {
                const { code } = await mRequest.delete(
                  api.common.deleteAttachmentByKey(this.qrKey, file.attachmentId),
                  {},
                  { methods: 'delete' },
                )
