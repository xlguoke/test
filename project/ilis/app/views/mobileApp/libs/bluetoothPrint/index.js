...
export function sendDataToDevice(options) {
  ...
  const speed = options.onceLength
...
export function printQR(options) {
  // 打印二维码的十进制指令data：
  const data = [29, 107, 97, 7, 4, options.value.byteLength, 0]
  sendDataToDevice({
    ...options,
...
export function overwriteImageData(data) {
  const sendWidth = data.width
  const sendHeight = data.height
...
export function CreatCPCLPage(width, height, printNum, rotation = 0, offset = 0) {
  let strCmd = `! ${offset} 200 200 ${height} ${printNum}\n`
...
export function addCPCLQRCode(x, y, level, ver, scale, content) {
  let strCmd = `B QR ${x} ${y} M ${ver} U ${scale}\n${level}A,${content}\n`
  strCmd += 'ENDQR\n'
  return strCmd
};
...
export function addCPCLSETMAG(scaleX, scaleY) {
  const strCmd = `SETMAG ${scaleX} ${scaleY}\n`
  return strCmd
};
...
export function addCPCLInverseLine(x0, y0, x1, y1, width) {
  const strCmd = `IL ${x0} ${y0} ${x1} ${y1} ${width}\n`
  return strCmd
};
...
export function addCPCLLine(x0, y0, x1, y1, width) {
  const strCmd = `L ${x0} ${y0} ${x1} ${y1} ${width}\n`
  return strCmd
};
...
export function addCPCLBox(x0, y0, x1, y1, width) {
  const strCmd = `BOX ${x0} ${y0} ${x1} ${y1} ${width}\n`
  return strCmd
};
...
export function addCPCLImageCmd(x, y, data) {
  let strImgCmd = ''
  const threshold = data.threshold || 180
  const myBitmapWidth = data.width
  const myBitmapHeight = data.height
  const len = Number.parseInt((myBitmapWidth + 7) / 8)// 一行的数据长度
  // console.log('len=',len);
  ...
  for (i = 0; i < myBitmapHeight; i++) {
    ...
    for (j = 0; j < myBitmapWidth; j++) {
      ...
      if (grayPixle1 < threshold)
        sendImageData[ndata + Number.parseInt(j / 8)] |= (0x80 >> (j % 8))
    }
    ndata += len
  }
