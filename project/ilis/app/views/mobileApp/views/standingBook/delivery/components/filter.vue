...
export default {
  ...
  methods: {
    ...
    onSortSelect(row, index) {
      ...
      this.formData.queryStatus = row.name
    ...
    handleInit(val) {
      ...
      const deliveryArr = [deliveryStart, deliveryEnd]
        .filter(i => i)
        .map(i => formatDate(i))
      if (val.queryStatus === '未到期' || val.queryStatus === 1) {
        this.formData.queryStatus = '未到期'
      }
      else if (val.queryStatus === '已到期' || val.queryStatus === 2) {
        this.formData.queryStatus = '已到期'
      }
      else if (val.queryStatus === '已处理' || val.queryStatus === 3) {
        this.formData.queryStatus = '已处理'
      }
      else {
        this.formData.queryStatus = '全部'
      }
