      ...
      <div class="sampleDetail-receiveTest">
        <van-form @submit="getSample">
          ...
          <FormItemInput
            ...
            required
            placeholder="请输入"
            :rules="[{ required: true, message: '请输入分包数量' }]"
            @input="changeAmount"
          />
          ...
          <template v-if="formData.type === '收样处留样'">
            <FormItemInput
...
export default {
  ...
  created() {
    ...
    this.getSubcontractUnitList()
    this.getBaseInfo()
  },
  methods: {
    // 获取分包单位
    getSubcontractUnitList() {
      mRequest({
        method: 'GET',
        url: api.samples.subcontractUnitList,
        params: {
          page: 1,
          size: 1000,
        },
        ...
        if (res.code === 20000) {
          ...
          this.companyList = rows ? rows.map(i => i.unitName) : []
        }
      })
    },
    getBaseInfo() {
