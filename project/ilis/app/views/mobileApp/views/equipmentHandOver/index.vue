  ...
  <div class="equipment">
    <van-sticky>
      ...
      <van-tabs v-model:active="activeName" @click="onClickTab">
        <van-tab title="待安装设备" :name="1"></van-tab>
        <van-tab title="待移交设备" :name="2"></van-tab>
      </van-tabs>
    ...
    >
      ...
      >
        ...
        <template #footer>
          <ListCardAction>
            <van-button size="small" @click="goCallinForm(item)">
              详情
            </van-button>
          </ListCardAction>
        </template>
...
export default {
  ...
  methods: {
    ...
    goCallinForm(item) {
      ...
      else if (item.transferStatus === '40') {
        this.$router.push({
    ...
    onClickTab() {
      if (this.activeName === 1) {
        this.transferStatus = 30
      }
      else {
        this.transferStatus = 40
      }
      this.resetPage()
    },
    getEqRecord() {
      mRequest
        .get('rest/eqTransfer/user/call-in/list', {
