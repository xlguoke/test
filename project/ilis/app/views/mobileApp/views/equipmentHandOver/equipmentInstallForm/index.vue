  ...
  <div>
    ...
    <van-form>
      <van-cell-group inset>
        ...
        >
          <!-- <template #button>
              <van-button size="small" type="primary" @click="projectShow = true">选择</van-button>
            </template> -->
        </van-field>

        <van-field
          v-model="selInstallPerson.userName"
          required
          label="安装人员"
          placeholder="安装人员"
          readonly
        >
          <template #button>
            <van-button size="small" type="primary" @click="openInstallPerson">
              选择
            </van-button>
          </template>
        </van-field>
        ...
        <FormItemInput
          v-model:value="installDescription"
          label="安装说明"
          placeholder="请输入安装说明"
        />

        <FormItemInput
          v-model:value="installRemark"
          label="备注"
          placeholder="请输入备注"
        />
    ...
    <van-popup v-model:show="installPersonShow" position="bottom">
      <van-picker
        ...
        @cancel="installPersonShow = false"
        @confirm="selInstallPersonConfirm"
      />
    </van-popup>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      selProject: {},
      formatDate,
      installTime: formatDate(new Date(), 'YYYY-MM-DD'),
      callOutRemark: '',
      fileList: [],
      abnormal: '0',
  ...
  methods: {
    ...
    getInstallPerson() {
      const url = `rest/userRelation/list?objectId=${this.selProject.id}`
    ...
    submitCallIn() {
      ...
      const params = {
        ...
        installUser: this.selInstallPerson.userName,
        installUserId: this.selInstallPerson.userId,
        installContact: this.installContact,
        installRemark: this.installRemark,
        installTime: `${this.installTime} 00:00:00`,
      }
