  ...
  methods: {
    async getDetail() {
      ...
      const res = await mRequest
        .get('/buyApplyController.do?getById', {
          params: {
    ...
    async getEqList() {
      const columnRes = await mRequest.get(
        'rest/common/custom-properties?customizeType=equipBuyApplyEdit',
      )

      const res = await mRequest.get(
        '/buyApplyController.do?datagridApplyDetail',
        {
          params: {
            buyApplyId: this.businessKey,
          },
      ...
      if (res.code !== 20010) {
        this.eqList = res.rows.map(item => [
          ...
          ...(columnRes.data || []).map((cItem) => {
            const obj = { label: cItem.columnName, value: '' }
            const vItem = (item.customizeValues || []).find(
              i => i.columnId === cItem.id,
            )
            if (vItem) {
              obj.value = vItem.columnValue
            }
            return obj
          }),
        ])
