...
<template>
  ...
  >
    <div class="min-h-[50vh]">
      <a-alert message="提示" type="info" show-icon class="p-3">
        <template #description>
          <div>如果需要合并规程，请在以下重复规程数据中选择其中一条，系统自动将其它未选中的重复数据合并为您选择的数据</div>
          <div>当数据量较大时，合并操作将会比较耗时，请耐心等待</div>
        </template>
      </a-alert>
      <a-table
...
import type { MergeStandard } from '../api'
import { message } from 'ant-design-vue'
import { ref } from 'vue'
import { mergeStandardApi, syncBaseStandardApi } from '../api'
