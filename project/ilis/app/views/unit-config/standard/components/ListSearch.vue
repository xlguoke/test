  ...
  >
    ...
    <a-form-item>
      <a-space>
        ...
        <a-checkbox v-model:checked="checkedMark" class="ml-6" @change="changeCheckMark">
          只查看“待验证”规程
        </a-checkbox>
      </a-space>
    </a-form-item>
...
import { BOOLEAN_DICT, BOOLEAN_OPT, SOURCE_TYPE_DICT } from '..'
...
const form = reactive({
  standardTypeId: '',
  quickQryParam: '',
  onlyViewPendingVerifyStandard: false,
})
...
function changeCheckMark() {
  emits('search', { ...form, onlyViewPendingVerifyStandard: checkedMark.value })
}
...
async function resetSearch() {
  ...
  emits('reset', { ...form, onlyViewPendingVerifyStandard: checkedMark.value })
}
...
function handleSearch2() {
  emits('search', { ...form2, onlyViewPendingVerifyStandard: checkedMark.value })
  cancel()
}
async function resetSearch2() {
  await advFormRef.value.resetFields()
  emits('reset', { ...form2, onlyViewPendingVerifyStandard: checkedMark.value })
}
</script>
