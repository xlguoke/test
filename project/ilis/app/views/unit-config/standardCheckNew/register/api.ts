...
export function waitCheckNewStandardListHitekCenter(params: WaitCheckNewStandardListParam) {
  const q: any = { ...params }
  let query = ''
  if (q.statuses && q.statuses.length > 0 && typeof q.statuses === 'object') {
    query = q.statuses.map((v: string) => `statuses=${v}`).join('&')
  }
  delete q.statuses
  for (const k in q) {
    const v = q[k] || ''
  ...
  return IlisApiHelper.get<any>(`rest/standard/check/new/select-hitek-center-check-new?${query}`)
}
