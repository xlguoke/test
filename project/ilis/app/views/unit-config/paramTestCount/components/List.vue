<template>
  <div class="h-full flex flex-col">
    ...
    <a-space>
      <a-button :disabled="!bigCategoryId" class="mt-4" @click="settingTestCountBySample">
        {{ term('按样品维护试验数量') }}
      </a-button>
...
import SettingSampleTestQuantity from './SettingSampleTestQuantity.vue'
...
const columns: ColumnType[] = [
  { title: '试验项目名称', dataIndex: 'testItemName', width: '12.5%' },
  { title: '试验参数系统名称', dataIndex: 'name', width: '12.5%' },
  { title: '试验参数显示名称', dataIndex: 'displayName', width: '12.5%' },
  { title: '检测类别', dataIndex: 'paramCategoryName', width: '12.5%' },
  { title: '试验数量', dataIndex: 'testQuantity', width: '12.5%' },
  { title: '共用试验数量名称', dataIndex: 'shareTestQuantityIdent', width: '12.5%' },
  { title: '共用试验数量', dataIndex: 'shareTestQuantity', width: '12.5%' },
  { title: `${term('按样品维护试验数量')}`, dataIndex: 'hasSampleTestQuantity', width: '12.5%', customRender: ({ record }) => record.hasSampleTestQuantity ? '是' : '否' },
]
...
function settingTestCountBySample() {
  ...
  if (selectedRowKeys.value.length > 1) {
    message.warn(`${term('按样品设置试验参数数量只能选择一个参数！')}`)
    return
  }

  AnyDialogHelper.showModel(SettingSampleTestQuantity, {
    bigCategoryId: bigCategoryId.value,
    paramId: selectedRowKeys.value[0],
  }).then(() => {
    getList()
  })
}
