<template>
  ...
  >
    ...
    <a-space direction="vertical">
      ...
      <a-button v-if="isDetail" @click="handleReturn(selectedRowKeys)">
        批量确认取回
      </a-button>
      <a-table
        row-key="id"
        ...
        :row-selection="{
          ...
          getCheckboxProps: (record: any) => ({
            disabled: record.status === SentStatus.END,
          }),
        }"
        class=" w-full"
      >
        <template #bodyCell="{ column, record, text, index }">
          ...
          <template v-if="column.dataIndex === 'status'">
            <span>{{ SentStatusDict.getLabelByKey(record.status) }}</span>
          </template>
          ...
          <template v-if="column.dataIndex === 'operation' && isDetail">
            <a-button v-if="record.status === SentStatus.CREATE" type="link" size="small" @click="handleReturn([record.id])">
              确认取回
            </a-button>
          </template>
        </template>
...
import { addColumns, CheckSendStatus, CheckSendStatusDict, SentStatus, SentStatusDict } from '..'
import { addCheckSend, getCheckSendDetail, getCheckSendDetailDeviceList, getParamsByDeviceId, returnCheckItems } from '../api'
...
const rules: Record<string, Rule[]> = {
  batchNumber: [
    { required: true, message: '请输入送检批号' },
    { max: 200, message: '最多200个字符', trigger: 'change' },
  ],
  unit: [
    { required: true, message: '请输入检校单位' },
    { max: 200, message: '最多200个字符', trigger: 'change' },
  ],
  sendTime: [
...
function handleReturn(ids: string[]) {
  if (!ids?.length) {
  ...
  Modal.confirm({
    ...
    centered: true,
    onOk: async () => {
      await returnCheckItems(ids)
      message.success('取回成功')
      init()
    },
  })
}
