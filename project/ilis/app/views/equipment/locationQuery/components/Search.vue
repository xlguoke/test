<template>
  <div class="absolute right-5 top-5 p-3 gap-3 bg-[var(--colorBgContainer)] rounded">
    <a-spin :spinning="loading">
      ...
      <div v-if="showResultList.length" class="flex flex-col w-[380px] h-[75vh]">
        <div v-show="showHistory">
          <p
            class="mb-2 text-colorPrimary cursor-pointer hover:opacity-80"
            @click="backToResult"
          >
            &lt; 返回上一级搜索结果
          </p>
          <p class="flex justify-between p-2 bg-[var(--colorBgContainer)]">
        ...
        <ul class="mt-2 overflow-y-auto flex-1">
          <li
            ...
            :class="d.index === activeIndex ? 'bg-[var(--colorFillContent)]' : 'hover:bg-[var(--colorFillContentHover)]'"
            class="relative rounded mb-2 p-2 border border-[var(--colorBorder)] overflow-hidden"
            @click="clickDataItem(d, d.index)"
          >
            <div class="cursor-default">
              <div class="text-sm flex items-center">
                <p v-if="showHistory" class="mr-2 location-icon text-center text-xs">
              ...
              <p class="mt-1 flex">
                <span class="w-16 text-right whitespace-nowrap">设备编号：</span>
                <span class="flex-1 w-0">{{ d.equipmentSn || '-' }}</span>
              </p>
              <p class="flex">
                <span class="w-16 text-right whitespace-nowrap">上报地址：</span>
                <span class="flex-1 w-0">{{ d.locate }}</span>
              </p>
              <p class="flex">
                <span class="w-16 text-right whitespace-nowrap">经纬坐标：</span>
                <span class="flex-1 w-0">{{ d.locateLongitude }},{{ d.locateLatitude }}</span>
              </p>
              <p class="flex justify-between gap-4">
                <span class="flex justify-start"><span class="w-16 text-right whitespace-nowrap">上报人：</span>{{ d.escalationUser }}</span>
                <span>上报时间：{{ d.escalationTime }}</span>
              </p>
            </div>
            <a
              v-if="!showHistory"
              class="block mt-2 pt-1 w-full !text-colorPrimary text-center border-t border-[var(--colorBorder)]"
