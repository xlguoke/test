<template>
  <div>
    ...
    >
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.dataIndex === 'value'">
              ...
              <a-select v-else v-model:value="record.value" class="w-full" placeholder="请选择核查结果">
      ...
      <div class="mt-4">
        <a-textarea v-model:value="formState.otherCheckResult" placeholder="请输入其他检查及检查结果" :rows="4" />
      </div>
      <template #footer>
...
import { BUSINES_TYPE } from '~/components/htUploadFile/index.ts'
...
async function getCheckItems() {
  ...
  checkItemList.value = res.data.obj?.map((item) => {
    item.value = true
    return item
  })
}
