  ...
  <div>
    ...
    >
      <a-spin tip="加载中..." :spinning="detailLoading">
        ...
        <a-alert v-if="!isDetail" message="核查部门、核查人需填写完整后，才允许提交审核" type="info" show-icon class="!mt-2" />
...
async function onSelectEq() {
  ...
  rows.forEach((item) => {
    if (!originDeviceList.value.find(i => i.equipmentId === item.equipmentId)) {
      ...
      eqItem.checkDepartmentId = item.departmentId
      eqItem.checkDepartment = item.department
