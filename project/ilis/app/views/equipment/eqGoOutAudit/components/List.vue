      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.dataIndex === 'egressTime'">
            {{ EQUIPMENT_OUTINGS_OUTING_TIME_CONFIGURINGS ? record.egressTime : dayjs(record.egressTime).format('YYYY-MM-DD') }}
          </template>
...
import type { PropsParam } from './AddEdit.vue'
...
import { EGRESS_APPLY_STATUS, EqGoOutAuditEntity, EqGoOutAuditType } from '..'
...
const quickTypeOptions = [
  ...
  { label: '设备编号', value: 'equipmentQuery' },
]
...
function customHandleSearch(data: any) {
  if (data.createDate && data.createDate[0]) {
...
async function handleSubmit(record: EqGoOutAuditEntity) {
  ...
  await AnyDialogHelper.showModel(ProcessModal, {
    processType: record.type === EqGoOutAuditType.EGRESS ? ProcessType.EQ_EGRESS_APPLY : ProcessType.EQ_EGRESS_TRANSITION_APPLY,
    processId: record.id,
    hideRemark: true,
    submitAuditApi: auditApi,
    defaultAuditers: record.type === EqGoOutAuditType.EGRESS
      ? undefined
      : {
          borrow_user_audit: [{
            presetUserId: record.transitionUserId,
            presetUsername: record.transitionUserName,
            presetUserRealName: record.transitionUser,
          }],
        },
