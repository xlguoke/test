<template>
  <ht-modal
    ...
    :title="`${param?.editType === 'add' ? '新增' : '编辑'}`"
    width="800px"
    :loading="spinning"
...
import { EgressStatus, EgressStatusDict } from '~/views/equipmentNew/egress/eqEgressList/list/OperationEntity'
import { EqGoOutAuditType } from '../'
...
const isAllowedOutsideDevice = ref(false)

const allowedStatus = ref<EgressStatus[]>([])
...
onMounted(async () => {
  ...
  isAllowedOutsideDevice.value = await getBusinessParam('EQ_EGRESS_TRANSITION')
  if (isAllowedOutsideDevice.value) {
    allowedStatus.value = [
...
const payload = computed(() => {
  if (props.param?.editType === 'add') {
    return { egressStatus: '10,30', transitionQuery: 1 }
  }
  else {
    return formData.value?.type === EqGoOutAuditType.EGRESS
      ? { egressStatus: '10,30' }
      : { egressStatus: '40,60,70,80,90,120', transitionUserId: formData.value?.transitionUserId }
  }
})
...
/** 选择设备回调 */
function selectedEquipment(datas: DeviceEntity[]) {
  if (isAllowedOutsideDevice.value) {
    const allowed = datas.filter(d => allowedStatus.value.includes(d.egressStatus))
    if (!allowed?.length) {
      message.warning(`仅允许选择${allowedStatus.value.map(s => EgressStatusDict.getLabelByKey(s)).join('、')}的设备`)
      return
    }
  }
  else {
...
async function addEquipmentByCode() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    else if (!allowedStatus.value.includes(d.egressStatus)) {
