        ...
        <template #planYear="{ data }">
          <div class="flex gap-2">
            ...
            <a-form-item
              ...
              class="w-[50%] overflow-visible m-0"
              name="planMonth"
            >
              <IlisInput
        ...
        >
          ...
          >
            <template #headerCell="{ column, title }">
              <template v-if="['upkeepProjectName', 'planPeriod'].includes(column.dataIndex as string)">
...
async function handleOk() {
  ...
  const isAllFinish = dataSourceAll.value?.every((i) => {
    if (initData.value.type === EquipmentUpkeepPlanType.MONTH) {
      return i.upkeepProjectName
    }
    else {
      return i.upkeepProjectName && i.planPeriod
    }
  })
  if (!isAllFinish) {
