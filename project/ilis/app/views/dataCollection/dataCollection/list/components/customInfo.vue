...
import addConfigField from './addCustomField'
...
export default {
  ...
  methods: {
    ...
    getFields() {
      this.loading = true
      window.$oAjax
        .get(window.$oApi.dataCollection.customFields, {
          params: { type: 'data' },
        })
        .then((res) => {
          if (res && res.success) {
    ...
    cancelModal() {
      if (this.isEdit) {
        window.$oAntdModal.warning(window.$oMsgTips.info('请先保存或取消当前正在编辑项'))
        return
      }
      this.callback()
      this.visible = false
    ...
    deleteRow(data) {
      window.$oAntdConfirm({
        ...
        onOk: () => {
          window.$oAjax
            .get(window.$oApi.dataCollection.deleteCustomField, {
              params: { id: data.id },
            })
            .then((res) => {
              if (res.success) {
