          ...
          >
            ...
            <template #bodyCell="{ record, column }">
              ...
              <template v-if="column.dataIndex === 'alias'">
                <span v-if="isDetail">{{ record.alias }}</span>
                <a-input
                  v-else
                  v-model:value.trim="record.alias"
                  :maxlength="50"
                  placeholder="请输入"
                />
              </template>
...
import { getColumnsByType, getRequestType, RequestType } from '~/views/common/purchaseRequest'
