<template>
  <div class="flex flex-col" style="height: calc(100% + 16px)">
    ...
    <a-space class="mt-4">
      <a-button v-permission="'purchase::apply::edit'" type="primary" @click="onAdd">
        新增
      </a-button>
      <a-button v-permission="'purchase::apply::submit'" @click="onBatchSubmit">
        提交
      </a-button>
      <a-button v-permission="'purchase::apply::export'" :loading="exportLoading" @click="onExport">
        导出
      </a-button>
      <IlisCustomColumns
        ...
        >
          <template #bodyCell="{ record, column }">
            ...
            <template v-if="column.dataIndex === 'Action'">
              <a-button v-if="![RequestStatusType.审核中, RequestStatusType.已完成].includes(record.status)" v-permission="'purchase::apply::edit'" type="link" @click.stop="onEdit(record)">
                编辑
              </a-button>
              <a-button v-if="[RequestStatusType.待提交, RequestStatusType.未通过].includes(record.status)" v-permission="'purchase::apply::submit'" type="link" @click.stop="onSubmit(record)">
                提交
              </a-button>
              <a-button type="link" @click.stop="onCheckDetail(record)">
                详情
              </a-button>
              <a-button v-if="![RequestStatusType.审核中, RequestStatusType.已完成].includes(record.status)" v-permission="'purchase::apply::delete'" type="link" danger @click.stop="onDel(record)">
                删除
              </a-button>
