...
import { humitureControll, humitureTemSet } from '~/views/common/humiture/display/api.ts'
...
// const refreshDetail = inject<() => void>('refreshDetail', () => {})
...
// 初始化设置的温度范围或者湿度范围
const settingFormState = computed(() => {
  if (props.type === 'tem') {
    return {
      min: statusData.value?.minIotTem || dataSource.value?.minTemperature,
      max: statusData.value?.maxIotTem || dataSource.value?.maxTemperature,
    }
  }
  else {
    return {
      min: statusData.value?.minIotHum || dataSource.value?.minHumidity,
      max: statusData.value?.maxIotHum || dataSource.value?.maxHumidity,
    }
  }
})
...
const status = computed(() => {
  ...
  if (isDefined(val.value)) {
    if (props.type === 'tem') {
      if (!isDefined(dataSource.value?.minTemperature) || !isDefined(dataSource.value?.maxTemperature)) {
        return ''
      }
      ...
      if (!isDefined(dataSource.value?.minHumidity) || !isDefined(dataSource.value?.maxHumidity)) {
        return ''
      }
...
async function onOpen() {
  ...
  const record = dataSource.value
  const data: IotDeviceThreshold = {
    iotEqId: record.labIotEq.iotEqId,
    laboratoryId: record.id,
    type: props.type, // 'tem' : 'hum'
    minTem: record.minTemperature ?? 20,
    maxTem: record.maxTemperature ?? 30,
    minHum: record.minHumidity ?? 30,
    maxHum: record.maxHumidity ?? 80,
  }
  ...
  try {
    ...
    if (props.type === 'tem') {
      ...
      settingFormState.value.min = data.minTem
      settingFormState.value.max = data.maxTem
    }
    else {
      statusData.value.humIotStatus = true
      settingFormState.value.min = data.minHum
      settingFormState.value.max = data.maxHum
    }
