<template>
  <a-flex class="h-full" vertical>
    <a-space>
      <a-button
        v-permission="'humiture_display_open'" type="primary"
        :disabled="(statusData.temIotStatus && statusData.humIotStatus)"
        @click="onOneTouch"
      >
        一键开启
      </a-button>
...
import { getIotLabDetail, getIots } from '~/views/common/humiture/displayDetails/api.ts'
...
const statusData = ref(props.statusData || {})
...
async function onOneTouchCallback() {
  const displayId = dataSource.value.id
  const res = await getIotLabDetail(displayId)
  const temIotStatus = !!((res.data.minIotTem && res.data.maxIotTem))
  const humIotStatus = !!((res.data.minIotHum && res.data.maxIotHum))
  const newData = {
    minIotTem: res.data.minIotTem,
    maxIotTem: res.data.maxIotTem,
    minIotHum: res.data.minIotHum,
    maxIotHum: res.data.maxIotHum,
  }
  statusData.value = {
    temIotStatus,
    humIotStatus,
