                  ...
                  <template v-else>
                    <div v-if="record.editable">
                      ...
                      <span
                        v-else-if="
                          record[`${column.dataIndex}_obj`]
                            && record[`${column.dataIndex}_obj`].type == 'textArea'
                        "
                      >
                        <a-textarea
                          :value="text"
...
import QuoteEqBuPlanModal from '~/views/equipmentNew/buy/equipBuyApplyList/list/components/QuoteEqBuPlanModal.vue'
  ...
  methods: {
    ...
    async showModal(editId, isDetail) {
      ...
      if (editId) {
        await window.$oAjax({
          method: 'get',
          url: window.$oApi.equipBuyApplyList.detailById,
          params: { buyApplyId: editId },
        }).then(async (res) => {
          if (res.success) {
            ...
            await window.$oAjax({
              method: 'get',
              url: window.$oApi.equipBuyApplyList.detailDatagrid,
              params: { buyApplyId: editId },
            }).then((res) => {
              if (res.total >= 0) {
    ...
    handleOk() {
      this.$refs.formRef.validateFields().then(() => {
        ...
        // data.applyDetailStr = JSON.stringify(this.dataSource);
        if (this.dataObj.id) {
        ...
        if (this.dataRequired(data)) {
          ...
          for (let i = 0; i < dataSource.length; i++) {
            ...
            // item.customValues ? item.customValues = [...item.customValues, ...customValues] : "";

            newDataSource.push({ ...item })
          }

          data.applyDetailStr = JSON.stringify(newDataSource)
          this.spinning = true
          window.$oAjax({
            method: 'POST',
            url: window.$oApi.equipBuyApplyList.savePlan,
    ...
    onQuote() {
      AnyDialogHelper.showModel(QuoteEqBuPlanModal, {}).then((rows) => {
        this.dataSource = [...this.dataSource, ...rows.map(i => ({
          id: generateGUID(),
          name: i.name,
          standard: i.standard,
    ...
    setOrg(record) {
      ...
      this.$refs.AddOrg.showModal('radio', 'depart')
    },
    getOrg(ids, orgData) {
      const newData = [...this.dataSource]
