          ...
          <a-col style="width: 240px">
            ...
            >
              ...
              <span v-else-if="durationType == 2000">
                <a-month-picker style="width: 100%" @change="changeRange" />
              </span>
              <span v-else-if="durationType == 3000">
                <a-week-picker style="width: 100%" @change="changeRange" />
              </span>
            </a-form-item>
          </a-col>
          <a-col>
...
export default {
  ...
  methods: {
    ...
    getData(params) {
      this.loading = true
      window.$oAjax
        .get(window.$oApi.chargeStatistic.getData, { params })
        .then((res) => {
          if (res.code && res.code === 20000) {
            ...
            if (durationType == 1000) {
              ...
              chartOption.xAxis.data = resData.map(item => item.month)
            }
            else {
              this.columns[0].dataIndex = 'day'
              chartOption.xAxis.data = resData.map(item => item.day)
            }
            chartOption.series[0].data = resData.map(item => item.totalPaid)
