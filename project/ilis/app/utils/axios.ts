...
ilisAxios.interceptors.request.use((config) => {
  try {
...
function MalformedResponseTransformer(data: any, headers: AxiosResponseHeaders) {
  ...
  else if (data.code === 20050) {
    const originalText = data.message || data.msg || '请求错误'
    const message = originalText
      .split(/<br\s*\/?>/i)
      .filter((line: any) => line.trim() !== '')
    Modal.warning({
      title: '提示',
      content: h('div', {}, message.map((i: any) => { return h('div', {}, i) })),
      centered: true,
      okText: '确定',
    })
    data.code = 20000
