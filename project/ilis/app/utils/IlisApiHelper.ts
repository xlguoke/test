...
export interface IlisApiHelperConfig {
  ...
  type?: IlisApiType
  /** # 是否允许创建字段：createBy、createDate、createName字段 */
  allowFieldCreate?: boolean
  /** # 是否允许更新字段：updateBy、updateDate、updateName字段 */
  allowFieldUpdate?: boolean
}
...
export abstract class IlisApiHelper {
  ...
  private static createApi<T>(config: IlisApiHelperConfig) {
    ...
    if (!config.config?.allowFieldCreate) {
      delete safeParams.createBy
      delete safeParams.createDate
      delete safeParams.createName
    }
    if (!config.config?.allowFieldUpdate) {
      delete safeParams.updateBy
      delete safeParams.updateDate
      delete safeParams.updateName
    }
   ...
   * 传入的参数会被转为url参数
   */
  static get<RES>(url: string, params?: Record<string, any>, config?: AxiosRequestConfig<any>) {
   ...
   * 传入的参数会被转为body参数
   */
  static post<RES>(url: string, params?: Record<string, any>, config?: AxiosRequestConfig<any>) {
   ...
   * 传入的参数会被转为form参数
   */
  static postForm<RES>(url: string, params?: Record<string, any>, config?: AxiosRequestConfig<any>) {
   ...
   * 传入的参数会被转为body参数
   */
  static put<RES>(url: string, params?: Record<string, any>, config?: AxiosRequestConfig<any>) {
   ...
   * 传入的参数会被转为body参数
   */
  static delete<RES>(url: string, params?: Record<string, any>, config?: AxiosRequestConfig<any>) {
