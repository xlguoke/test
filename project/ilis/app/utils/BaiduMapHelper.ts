...
export class BaiduMapHelper {
  // 缓存的当前定位数据
  private static positionData?: PositionDTO

  /**
   * 获取当前位置经纬度
   */
  static getCurrentPoint(): Promise<PositionDTO> {
    return new Promise((resolve, reject) => {
      ...
      geolocation.getCurrentPosition(function (r: PositionDTO) {
        if ((this as unknown as any).getStatus() === window.BMAP_STATUS_SUCCESS) {
  ...
  static async getLocation(longitude: string | number, latitude: string | number) {
    ...
    return new Promise((resolve, reject) => {
      geocoder.getLocation(point, (result: LocationDTO) => {
        if (result) {
          resolve(result)
        }
        else {
