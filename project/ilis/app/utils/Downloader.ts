...
class Downloader {
   ...
   * 从响应头中获取文件名称
   */
  getFileNameByHeaders(res: any) {
    try {
      const data = res.headers.get('content-disposition')
      return decodeURIComponent(data.split('fileName=')[1])
    }
    catch (e) {
  ...
  async requestBlob(url: string, method = 'get') {
    ...
    if (res) {
      ...
      return {
        blob,
        fileName: this.getFileNameByHeaders(res),
      }
    }
  }
  ...
  async getDownloadData(downloadData: string | Blob, downloadFilename: string) {
    ...
    if (typeof (downloadData) === 'string') {
      ...
      return {
        blob: res.blob,
        filename: downloadFilename || res.fileName,
      }
    }
