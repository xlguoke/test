...
import { getGroupConfigObj } from '../decorator/Group'
...
export class AnyBaseModel {
   ...
   * @returns 静态方法调用，返回实例方法调用
   */
  static getGroupConfigObj() {
    return new this().getGroupConfigObj()
  }
  ...
  /**
   * # 获取字段opiton配置选项（适用treeSelect）
   * @param {string} field 字段属性
   */
  getTreeOptions(field: string) {
    const opts = this.getFieldDictionaryArray(field) || []
    const list: TreeOption[] = []
    ;(function _each(datas: any[], _list) {
      datas.forEach((d) => {
        const opt = {
          ...d,
          label: d.label,
