...
import type { FormAttributes } from './IFormComponentAttributes'
import type { IInputSelectorConfig } from './IInputSelectorConfig'

/**
 * # 表单字段配置接口
 */
export interface IFormFieldConfig<T extends EFormItemType = EFormItemType> extends IFieldBaseConfig {
  /**
   * # 表单类型
  ...
  /**
   * # 表单属性（类型安全）
   * - 根据formType自动推断对应的组件属性类型
   */
  attributes?: FormAttributes<T>
  ...
  /**
   * # range类型数据字段映射
   * 适用{@link EFormItemType.INPUT_RANGE}
   * 适用{@link EFormItemType.DATE_RANGE}
   * 适用{@link EFormItemType.DATETIME_RANGE}
   */
  rangeMap?: [string, string]
}
