...
import Main from '~/views/cloudPlatform/platformDataCenter/platformDataCenter/App/index.vue'
import router from '~/views/cloudPlatform/platformDataCenter/platformDataCenter/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#platformDataCenter')

  if (!el) {
    console.warn('#platformDataCenter not found')
    return
  }
