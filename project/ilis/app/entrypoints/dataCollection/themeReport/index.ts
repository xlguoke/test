...
import Main from '~/views/dataCollection/themeReport/App/index.vue'
import router from '~/views/dataCollection/themeReport/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#themeReport')

  if (!el) {
    console.warn('#themeReport not found')
    return
  }
