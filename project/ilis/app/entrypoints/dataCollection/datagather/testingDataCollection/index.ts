...
import Main from '~/views/dataCollection/datagather/testingDataCollection/App/index.vue'
import router from '~/views/dataCollection/datagather/testingDataCollection/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testingDataCollection')

  if (!el) {
    console.warn('#testingDataCollection not found')
    return
  }
  ...
  app.mount('#testingDataCollection')
})()
