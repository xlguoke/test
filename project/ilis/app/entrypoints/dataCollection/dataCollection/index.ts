...
import Main from '~/views/dataCollection/dataCollection/App/index.vue'
import router from '~/views/dataCollection/dataCollection/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#dataCollection')

  if (!el) {
    console.warn('#dataCollection not found')
    return
  }
