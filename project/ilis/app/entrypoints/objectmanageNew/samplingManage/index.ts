...
import Main from '~/views/objectmanageNew/samplingManage/App/index.vue'
import router from '~/views/objectmanageNew/samplingManage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#samplingManage')

  if (!el) {
    console.warn('#samplingManage not found')
    return
  }
