...
import Main from '~/views/objectmanageNew/period/testPieceManage/App/index.vue'
import router from '~/views/objectmanageNew/period/testPieceManage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testPieceManage')

  if (!el) {
    console.warn('#testPieceManage not found')
    return
  }
