...
import Main from '~/views/objectmanageNew/period/storageSite/App/index.vue'
import router from '~/views/objectmanageNew/period/storageSite/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#storageSite')

  if (!el) {
    console.warn('#storageSite not found')
    return
  }
