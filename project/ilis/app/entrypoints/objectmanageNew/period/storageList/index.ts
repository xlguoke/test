...
import Main from '~/views/objectmanageNew/period/storageList/App/index.vue'
import router from '~/views/objectmanageNew/period/storageList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#storageList')

  if (!el) {
    console.warn('#storageList not found')
    return
  }
