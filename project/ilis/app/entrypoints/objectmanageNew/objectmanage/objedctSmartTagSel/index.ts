...
import Main from '~/views/objectmanageNew/objectmanage/objedctSmartTagSel/App/index.vue'
import router from '~/views/objectmanageNew/objectmanage/objedctSmartTagSel/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#objedctSmartTagSel')

  if (!el) {
    console.warn('#objedctSmartTagSel not found')
    return
  }
