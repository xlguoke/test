...
import Main from '~/views/fee/statistics/contractCostStatistic/App/index.vue'
import router from '~/views/fee/statistics/contractCostStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#contractCostStatistic')

  if (!el) {
    console.warn('#contractCostStatistic not found')
    return
  }
  ...
  app.mount('#contractCostStatistic')
})()
