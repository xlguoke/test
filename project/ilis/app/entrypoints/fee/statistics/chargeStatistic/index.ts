...
import Main from '~/views/fee/statistics/chargeStatistic/App/index.vue'
import router from '~/views/fee/statistics/chargeStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#chargeStatistic')

  if (!el) {
    console.warn('#chargeStatistic not found')
    return
  }
