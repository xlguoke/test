...
import Main from '~/views/fee/statistics/arrearsStatistic/App/index.vue'
import router from '~/views/fee/statistics/arrearsStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#arrearsStatistic')

  if (!el) {
    console.warn('#arrearsStatistic not found')
    return
  }
