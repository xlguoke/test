...
import Main from '~/views/fee/taskSettlementManag/taskSettlementManag/App/index.vue'
import router from '~/views/fee/taskSettlementManag/taskSettlementManag/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#taskSettlementManag')

  if (!el) {
    console.warn('#taskSettlementManag not found')
    return
  }
