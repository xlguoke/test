...
import Main from '~/views/fee/outputValueStatistics/stuffProduction/App/index.vue'
import router from '~/views/fee/outputValueStatistics/stuffProduction/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#stuffProduction')

  if (!el) {
    console.warn('#stuffProduction not found')
    return
  }
