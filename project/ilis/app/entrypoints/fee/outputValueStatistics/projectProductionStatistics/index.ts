...
import Main from '~/views/fee/outputValueStatistics/projectProductionStatistics/App/index.vue'
import router from '~/views/fee/outputValueStatistics/projectProductionStatistics/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#projectProductionStatistics')

  if (!el) {
    console.warn('#projectProductionStatistics not found')
    return
  }
  ...
  app.mount('#projectProductionStatistics')
})()
