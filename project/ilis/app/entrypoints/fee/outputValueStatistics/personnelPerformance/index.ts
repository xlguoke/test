...
import Main from '~/views/fee/outputValueStatistics/personnelPerformance/App/index.vue'
import router from '~/views/fee/outputValueStatistics/personnelPerformance/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#personnelPerformance')

  if (!el) {
    console.warn('#personnelPerformance not found')
    return
  }
  ...
  app.mount('#personnelPerformance')
})()
