...
import Main from '~/views/fee/outputValueStatistics/projectProduction/App/index.vue'
import router from '~/views/fee/outputValueStatistics/projectProduction/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#projectProduction')

  if (!el) {
    console.warn('#projectProduction not found')
    return
  }
