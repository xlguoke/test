...
import Main from '~/views/fee/outputValueStatistics/projectDetails/App/index.vue'
import router from '~/views/fee/outputValueStatistics/projectDetails/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#projectDetails')

  if (!el) {
    console.warn('#projectDetails not found')
    return
  }
