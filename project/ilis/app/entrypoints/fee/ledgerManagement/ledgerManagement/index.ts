...
import Main from '~/views/fee/ledgerManagement/ledgerManagement/App/index.vue'
import router from '~/views/fee/ledgerManagement/ledgerManagement/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#ledgerManagement')

  if (!el) {
    console.warn('#ledgerManagement not found')
    return
  }
