...
import Main from '~/views/supplierEvaluation/supplierEvaluation/App/index.vue'
import router from '~/views/supplierEvaluation/supplierEvaluation/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#supplierEvaluation')

  if (!el) {
    console.warn('#supplierEvaluation not found')
    return
  }
