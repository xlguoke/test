...
import Main from '~/views/datagather/acquisitionFieldConfiguration/App/index.vue'
import router from '~/views/datagather/acquisitionFieldConfiguration/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#acquisitionFieldConfiguration')

  if (!el) {
    console.warn('#acquisitionFieldConfiguration not found')
    return
  }
  ...
  app.mount('#acquisitionFieldConfiguration')
})()
