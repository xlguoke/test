...
import Main from '~/views/datagather/collectionFieldManagement/App/index.vue'
import router from '~/views/datagather/collectionFieldManagement/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#collectionFieldManagement')

  if (!el) {
    console.warn('#collectionFieldManagement not found')
    return
  }
  ...
  app.mount('#collectionFieldManagement')
})()
