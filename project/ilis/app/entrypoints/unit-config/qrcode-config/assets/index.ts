...
import qs from 'qs'
import IlisContainer from '~/components/ilisComponent/IlisContainer.vue'
...
import QrConfig from '~/views/reportManageNew/comm/qrCodeConfig/list/index.vue'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#assets-qrcode-config')

  if (!el) {
    console.warn('#assets-qrcode-config not found')
    return
  }

  const app = createApp(h(IlisContainer, {
    appId: 'assets-qrcode-config',
  }, [h(QrConfig, {
    pageCode: 'assetsQrCodeConfig',
  })]))
  ...
  app.mount('#assets-qrcode-config')
})()
