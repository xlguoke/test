...
import Main from '~/views/layerPage/taskSettlement/App/index.vue'
import router from '~/views/layerPage/taskSettlement/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#taskSettlement')

  if (!el) {
    console.warn('#taskSettlement not found')
    return
  }
