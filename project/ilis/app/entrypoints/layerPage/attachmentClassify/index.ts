...
import Main from '~/views/layerPage/attachmentClassify/App/index.vue'
import router from '~/views/layerPage/attachmentClassify/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#attachmentClassify')

  if (!el) {
    console.warn('#attachmentClassify not found')
    return
  }
