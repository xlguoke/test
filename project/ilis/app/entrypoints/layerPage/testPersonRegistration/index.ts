...
import Main from '~/views/layerPage/testPersonRegistration/App/index.vue'
import router from '~/views/layerPage/testPersonRegistration/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testPersonRegistration')

  if (!el) {
    console.warn('#testPersonRegistration not found')
    return
  }
  ...
  app.mount('#testPersonRegistration')
})()
