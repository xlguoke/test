...
import Main from '~/views/layerPage/specialCharacter/App/index.vue'
import router from '~/views/layerPage/specialCharacter/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#specialCharacter')

  if (!el) {
    console.warn('#specialCharacter not found')
    return
  }
