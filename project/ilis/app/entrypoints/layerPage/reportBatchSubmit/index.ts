...
import Main from '~/views/layerPage/reportBatchSubmit/App/index.vue'
import router from '~/views/layerPage/reportBatchSubmit/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportBatchSubmit')

  if (!el) {
    console.warn('#reportBatchSubmit not found')
    return
  }
