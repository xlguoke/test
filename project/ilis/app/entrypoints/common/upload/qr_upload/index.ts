...
import Main from '~/views/common/upload/qr_upload/App/index.vue'
import router from '~/views/common/upload/qr_upload/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#qr_upload')

  if (!el) {
    console.warn('#qr_upload not found')
    return
  }
