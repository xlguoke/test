...
import Main from '~/views/common/dataPublishConfig/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#dataPublishConfig')

  if (!el) {
    console.warn('#dataPublishConfig not found')
    return
  }
