...
import Main from '~/views/testbusiness/testitem/testItemDetail/App/index.vue'
import router from '~/views/testbusiness/testitem/testItemDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testItemDetail')

  if (!el) {
    console.warn('#testItemDetail not found')
    return
  }
