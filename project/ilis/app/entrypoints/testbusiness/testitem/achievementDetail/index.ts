...
import Main from '~/views/testbusiness/testitem/achievementDetail/App/index.vue'
import router from '~/views/testbusiness/testitem/achievementDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#achievementDetail')

  if (!el) {
    console.warn('#achievementDetail not found')
    return
  }
