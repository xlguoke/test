...
import Main from '~/views/testbusiness/testitem/testItem/App/index.vue'
import router from '~/views/testbusiness/testitem/testItem/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testItem')

  if (!el) {
    console.warn('#testItem not found')
    return
  }
