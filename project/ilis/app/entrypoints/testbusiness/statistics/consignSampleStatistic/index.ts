...
import Main from '~/views/testbusiness/statistics/consignSampleStatistic/App/index.vue'
import router from '~/views/testbusiness/statistics/consignSampleStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#consignSampleStatistic')

  if (!el) {
    console.warn('#consignSampleStatistic not found')
    return
  }
  ...
  app.mount('#consignSampleStatistic')
})()
