...
import Main from '~/views/testbusiness/statistics/consignInfoStatistic/App/index.vue'
import router from '~/views/testbusiness/statistics/consignInfoStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#consignInfoStatistic')

  if (!el) {
    console.warn('#consignInfoStatistic not found')
    return
  }
  ...
  app.mount('#consignInfoStatistic')
})()
