...
import Main from '~/views/testbusiness/consign/customColumn/App/index.vue'
import router from '~/views/testbusiness/consign/customColumn/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#customColumn')

  if (!el) {
    console.warn('#customColumn not found')
    return
  }
