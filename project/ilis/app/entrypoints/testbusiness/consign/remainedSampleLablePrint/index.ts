...
import Main from '~/views/testbusiness/consign/remainedSampleLablePrint/App/index.vue'
import router from '~/views/testbusiness/consign/remainedSampleLablePrint/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#remainedSampleLablePrint')

  if (!el) {
    console.warn('#remainedSampleLablePrint not found')
    return
  }
  ...
  app.mount('#remainedSampleLablePrint')
})()
