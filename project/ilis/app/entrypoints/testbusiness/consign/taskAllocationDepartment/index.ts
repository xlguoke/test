...
import Main from '~/views/testbusiness/consign/taskAllocationDepartment/App/index.vue'
import router from '~/views/testbusiness/consign/taskAllocationDepartment/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#taskAllocationDepartment')

  if (!el) {
    console.warn('#taskAllocationDepartment not found')
    return
  }
  ...
  app.mount('#taskAllocationDepartment')
})()
