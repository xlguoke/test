...
import Main from '~/views/testbusiness/consign/ConsignCustomProperty/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#ConsignCustomProperty')

  if (!el) {
    console.warn('#ConsignCustomProperty not found')
    return
  }
  ...
  app.mount('#ConsignCustomProperty')
})()
