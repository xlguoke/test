...
import Main from '~/views/testbusiness/consign/ageSpecimenTable/App/index.vue'
import router from '~/views/testbusiness/consign/ageSpecimenTable/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#ageSpecimenTable')

  if (!el) {
    console.warn('#ageSpecimenTable not found')
    return
  }
