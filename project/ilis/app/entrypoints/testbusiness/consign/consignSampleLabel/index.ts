...
import Main from '~/views/testbusiness/consign/consignSampleLabel/App/index.vue'
import router from '~/views/testbusiness/consign/consignSampleLabel/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#consignSampleLabel')

  if (!el) {
    console.warn('#consignSampleLabel not found')
    return
  }
