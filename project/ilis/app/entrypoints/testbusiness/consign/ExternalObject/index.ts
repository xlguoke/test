...
import Main from '~/views/testbusiness/consign/ExternalObject/App/index.vue'
import router from '~/views/testbusiness/consign/ExternalObject/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#ExternalObject')

  if (!el) {
    console.warn('#ExternalObject not found')
    return
  }
