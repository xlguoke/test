...
import Main from '~/views/testbusiness/standardMaterial/standardMaterial/App/index.vue'
import router from '~/views/testbusiness/standardMaterial/standardMaterial/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#standardMaterial')

  if (!el) {
    console.warn('#standardMaterial not found')
    return
  }
