...
import Main from '~/views/testbusiness/standardMaterial/standardPurchaseMaterial/App/index.vue'
import router from '~/views/testbusiness/standardMaterial/standardPurchaseMaterial/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#standardPurchaseMaterial')

  if (!el) {
    console.warn('#standardPurchaseMaterial not found')
    return
  }
  ...
  app.mount('#standardPurchaseMaterial')
})()
