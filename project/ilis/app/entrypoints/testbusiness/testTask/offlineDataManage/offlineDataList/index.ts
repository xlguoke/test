...
import Main from '~/views/testbusiness/testTask/offlineDataManage/offlineDataList/App/index.vue'
import router from '~/views/testbusiness/testTask/offlineDataManage/offlineDataList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#offlineDataList')

  if (!el) {
    console.warn('#offlineDataList not found')
    return
  }
