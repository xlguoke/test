...
import Main from '~/views/testbusiness/testTask/offlineDataManage/offlineSysLogs/App/index.vue'
import router from '~/views/testbusiness/testTask/offlineDataManage/offlineSysLogs/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#offlineSysLogs')

  if (!el) {
    console.warn('#offlineSysLogs not found')
    return
  }
