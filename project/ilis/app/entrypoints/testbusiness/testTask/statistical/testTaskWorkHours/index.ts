...
import Main from '~/views/testbusiness/testTask/statistical/testTaskWorkHours/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testTaskWorkHours/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testTaskWorkHours')

  if (!el) {
    console.warn('#testTaskWorkHours not found')
    return
  }
