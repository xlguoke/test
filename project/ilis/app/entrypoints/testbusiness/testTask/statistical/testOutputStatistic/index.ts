...
import Main from '~/views/testbusiness/testTask/statistical/testOutputStatistic/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testOutputStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testOutputStatistic')

  if (!el) {
    console.warn('#testOutputStatistic not found')
    return
  }
