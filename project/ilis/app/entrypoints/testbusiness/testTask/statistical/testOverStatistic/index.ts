...
import Main from '~/views/testbusiness/testTask/statistical/testOverStatistic/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testOverStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testOverStatistic')

  if (!el) {
    console.warn('#testOverStatistic not found')
    return
  }
