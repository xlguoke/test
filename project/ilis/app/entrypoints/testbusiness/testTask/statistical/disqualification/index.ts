...
import Main from '~/views/testbusiness/testTask/statistical/disqualification/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/disqualification/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#disqualification')

  if (!el) {
    console.warn('#disqualification not found')
    return
  }
