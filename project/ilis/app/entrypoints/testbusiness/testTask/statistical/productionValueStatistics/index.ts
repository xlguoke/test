...
import Main from '~/views/testbusiness/testTask/statistical/productionValueStatistics/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/productionValueStatistics/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#productionValueStatistics')

  if (!el) {
    console.warn('#productionValueStatistics not found')
    return
  }
  ...
  app.mount('#productionValueStatistics')
})()
