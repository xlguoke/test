...
import Main from '~/views/testbusiness/testTask/statistical/testTaskStatistic/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testTaskStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testTaskStatistic')

  if (!el) {
    console.warn('#testTaskStatistic not found')
    return
  }
