...
import Main from '~/views/testbusiness/testTask/statistical/testResultStatistic/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testResultStatistic/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testResultStatistic')

  if (!el) {
    console.warn('#testResultStatistic not found')
    return
  }
