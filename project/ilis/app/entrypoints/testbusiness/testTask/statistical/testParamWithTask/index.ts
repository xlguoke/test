...
import Main from '~/views/testbusiness/testTask/statistical/testParamWithTask/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testParamWithTask/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testParamWithTask')

  if (!el) {
    console.warn('#testParamWithTask not found')
    return
  }
