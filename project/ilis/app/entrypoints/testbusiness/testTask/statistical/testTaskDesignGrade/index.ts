...
import Main from '~/views/testbusiness/testTask/statistical/testTaskDesignGrade/App/index.vue'
import router from '~/views/testbusiness/testTask/statistical/testTaskDesignGrade/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testTaskDesignGrade')

  if (!el) {
    console.warn('#testTaskDesignGrade not found')
    return
  }
