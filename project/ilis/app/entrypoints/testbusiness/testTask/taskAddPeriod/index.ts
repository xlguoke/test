...
import Main from '~/views/testbusiness/testTask/taskAddPeriod/App/index.vue'
import router from '~/views/testbusiness/testTask/taskAddPeriod/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#taskAddPeriod')

  if (!el) {
    console.warn('#taskAddPeriod not found')
    return
  }
