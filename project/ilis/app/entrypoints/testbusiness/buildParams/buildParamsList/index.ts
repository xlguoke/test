...
import Main from '~/views/testbusiness/buildParams/buildParamsList/App/index.vue'
import router from '~/views/testbusiness/buildParams/buildParamsList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#buildParamsList')

  if (!el) {
    console.warn('#buildParamsList not found')
    return
  }
