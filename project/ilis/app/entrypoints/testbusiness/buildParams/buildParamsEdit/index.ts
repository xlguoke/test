...
import Main from '~/views/testbusiness/buildParams/buildParamsEdit/App/index.vue'
import router from '~/views/testbusiness/buildParams/buildParamsEdit/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#buildParamsEdit')

  if (!el) {
    console.warn('#buildParamsEdit not found')
    return
  }
