...
import Main from '~/views/testbusiness/testObject/snPreset/App/index.vue'
import router from '~/views/testbusiness/testObject/snPreset/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#snPreset')

  if (!el) {
    console.warn('#snPreset not found')
    return
  }
