...
import Main from '~/views/testbusiness/document/archivingDetail/App/index.vue'
import router from '~/views/testbusiness/document/archivingDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#archivingDetail')

  if (!el) {
    console.warn('#archivingDetail not found')
    return
  }
