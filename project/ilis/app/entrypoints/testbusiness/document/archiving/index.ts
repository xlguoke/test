...
import Main from '~/views/testbusiness/document/archiving/App/index.vue'
import router from '~/views/testbusiness/document/archiving/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#archiving')

  if (!el) {
    console.warn('#archiving not found')
    return
  }
