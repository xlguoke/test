...
import Main from '~/views/testbusiness/externalSample/externalSampleManage/App/index.vue'
import router from '~/views/testbusiness/externalSample/externalSampleManage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#externalSampleManage')

  if (!el) {
    console.warn('#externalSampleManage not found')
    return
  }
  ...
  app.mount('#externalSampleManage')
})()
