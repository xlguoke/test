...
import Main from '~/views/testbusiness/completionData/completionDataDetail/App/index.vue'
import router from '~/views/testbusiness/completionData/completionDataDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#completionDataDetail')

  if (!el) {
    console.warn('#completionDataDetail not found')
    return
  }
  ...
  app.mount('#completionDataDetail')
})()
