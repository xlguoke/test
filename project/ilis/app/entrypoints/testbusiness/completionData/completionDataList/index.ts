...
import Main from '~/views/testbusiness/completionData/completionDataList/App/index.vue'
import router from '~/views/testbusiness/completionData/completionDataList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#completionDataList')

  if (!el) {
    console.warn('#completionDataList not found')
    return
  }
