...
import Main from '~/views/testbusiness/completionData/completionDataConfig/App/index.vue'
import router from '~/views/testbusiness/completionData/completionDataConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#completionDataConfig')

  if (!el) {
    console.warn('#completionDataConfig not found')
    return
  }
  ...
  app.mount('#completionDataConfig')
})()
