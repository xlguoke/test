...
import Main from '~/views/system/sn/snModification/snModification/App/index.vue'
import router from '~/views/system/sn/snModification/snModification/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#snModification')

  if (!el) {
    console.warn('#snModification not found')
    return
  }
