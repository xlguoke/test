...
import Main from '~/views/system/subcontract/subcontractManage/App/index.vue'
import router from '~/views/system/subcontract/subcontractManage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#subcontractManage')

  if (!el) {
    console.warn('#subcontractManage not found')
    return
  }
