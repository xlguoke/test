...
import Main from '~/views/system/subcontract/sampleSubcontract/App/index.vue'
import router from '~/views/system/subcontract/sampleSubcontract/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#sampleSubcontract')

  if (!el) {
    console.warn('#sampleSubcontract not found')
    return
  }
