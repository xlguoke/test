...
import Main from '~/views/system/subcontract/unitSubcontract/App/index.vue'
import router from '~/views/system/subcontract/unitSubcontract/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#unitSubcontract')

  if (!el) {
    console.warn('#unitSubcontract not found')
    return
  }
