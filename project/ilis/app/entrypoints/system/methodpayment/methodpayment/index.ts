...
import Main from '~/views/system/methodpayment/methodpayment/App/index.vue'
import router from '~/views/system/methodpayment/methodpayment/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#methodpayment')

  if (!el) {
    console.warn('#methodpayment not found')
    return
  }
