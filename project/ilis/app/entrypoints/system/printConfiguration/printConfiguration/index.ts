...
import Main from '~/views/system/printConfiguration/printConfiguration/App/index.vue'
import router from '~/views/system/printConfiguration/printConfiguration/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#printConfiguration')

  if (!el) {
    console.warn('#printConfiguration not found')
    return
  }
