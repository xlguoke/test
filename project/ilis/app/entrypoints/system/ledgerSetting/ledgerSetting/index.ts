...
import Main from '~/views/system/ledgerSetting/ledgerSetting/App/index.vue'
import router from '~/views/system/ledgerSetting/ledgerSetting/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#ledgerSetting')

  if (!el) {
    console.warn('#ledgerSetting not found')
    return
  }
