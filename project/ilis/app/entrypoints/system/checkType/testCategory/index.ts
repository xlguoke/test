...
import Main from '~/views/system/checkType/testCategory/App/index.vue'
import router from '~/views/system/checkType/testCategory/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#testCategory')

  if (!el) {
    console.warn('#testCategory not found')
    return
  }
