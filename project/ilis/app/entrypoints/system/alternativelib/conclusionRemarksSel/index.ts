...
import Main from '~/views/system/alternativelib/conclusionRemarksSel/App/index.vue'
import router from '~/views/system/alternativelib/conclusionRemarksSel/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#conclusionRemarksSel')

  if (!el) {
    console.warn('#conclusionRemarksSel not found')
    return
  }
  ...
  app.mount('#conclusionRemarksSel')
})()
