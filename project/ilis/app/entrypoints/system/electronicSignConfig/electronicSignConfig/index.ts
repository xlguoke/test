...
import Main from '~/views/system/electronicSignConfig/electronicSignConfig/App/index.vue'
import router from '~/views/system/electronicSignConfig/electronicSignConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#electronicSignConfig')

  if (!el) {
    console.warn('#electronicSignConfig not found')
    return
  }
  ...
  app.mount('#electronicSignConfig')
})()
