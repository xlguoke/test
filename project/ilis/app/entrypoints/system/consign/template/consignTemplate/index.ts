...
import Main from '~/views/system/consign/template/consignTemplate/App/index.vue'
import router from '~/views/system/consign/template/consignTemplate/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#consignTemplate')

  if (!el) {
    console.warn('#consignTemplate not found')
    return
  }
