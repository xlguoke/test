...
import Main from '~/views/system/parameter/parameterVersion/App/index.vue'
import router from '~/views/system/parameter/parameterVersion/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#parameterVersion')

  if (!el) {
    console.warn('#parameterVersion not found')
    return
  }
