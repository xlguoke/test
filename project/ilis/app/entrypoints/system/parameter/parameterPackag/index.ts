...
import Main from '~/views/system/parameter/parameterPackag/App/index.vue'
import router from '~/views/system/parameter/parameterPackag/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#parameterPackag')

  if (!el) {
    console.warn('#parameterPackag not found')
    return
  }
