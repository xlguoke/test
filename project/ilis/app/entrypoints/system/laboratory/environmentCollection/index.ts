...
import Main from '~/views/system/laboratory/environmentCollection/App/index.vue'
import router from '~/views/system/laboratory/environmentCollection/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#environmentCollection')

  if (!el) {
    console.warn('#environmentCollection not found')
    return
  }
  ...
  app.mount('#environmentCollection')
})()
