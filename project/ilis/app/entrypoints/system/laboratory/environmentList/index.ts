...
import Main from '~/views/system/laboratory/environmentList/App/index.vue'
import router from '~/views/system/laboratory/environmentList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#environmentList')

  if (!el) {
    console.warn('#environmentList not found')
    return
  }
