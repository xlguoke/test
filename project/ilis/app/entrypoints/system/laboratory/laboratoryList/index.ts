...
import Main from '~/views/system/laboratory/laboratoryList/App/index.vue'
import router from '~/views/system/laboratory/laboratoryList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#laboratoryList')

  if (!el) {
    console.warn('#laboratoryList not found')
    return
  }
