...
import Main from '~/views/system/notice/noticeList/App/index.vue'
import router from '~/views/system/notice/noticeList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#noticeList')

  if (!el) {
    console.warn('#noticeList not found')
    return
  }
