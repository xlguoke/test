...
import Main from '~/views/system/notice/tSNoticeList/App/index.vue'
import router from '~/views/system/notice/tSNoticeList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#tSNoticeList')

  if (!el) {
    console.warn('#tSNoticeList not found')
    return
  }
