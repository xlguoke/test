...
import Main from '~/views/system/taskStatusMarker/taskStatusMarker/App/index.vue'
import router from '~/views/system/taskStatusMarker/taskStatusMarker/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#taskStatusMarker')

  if (!el) {
    console.warn('#taskStatusMarker not found')
    return
  }
