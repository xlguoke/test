...
import Main from '~/views/system/parameterScript/departmentParams/App/index.vue'
import router from '~/views/system/parameterScript/departmentParams/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#departmentParams')

  if (!el) {
    console.warn('#departmentParams not found')
    return
  }
