...
import Main from '~/views/system/parameterScript/parameterScript/App/index.vue'
import router from '~/views/system/parameterScript/parameterScript/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#parameterScript')

  if (!el) {
    console.warn('#parameterScript not found')
    return
  }
