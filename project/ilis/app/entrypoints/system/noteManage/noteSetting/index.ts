...
import Main from '~/views/system/noteManage/noteSetting/App/index.vue'
import router from '~/views/system/noteManage/noteSetting/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#noteSetting')

  if (!el) {
    console.warn('#noteSetting not found')
    return
  }
