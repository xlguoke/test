...
import Main from '~/views/system/noteManage/noteQuery/App/index.vue'
import router from '~/views/system/noteManage/noteQuery/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#noteQuery')

  if (!el) {
    console.warn('#noteQuery not found')
    return
  }
