...
import Main from '~/views/system/digitalLabel/electronicLabel/App/index.vue'
import router from '~/views/system/digitalLabel/electronicLabel/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#electronicLabel')

  if (!el) {
    console.warn('#electronicLabel not found')
    return
  }
