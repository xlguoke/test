...
import Main from '~/views/system/user/userBasicInfo/App/index.vue'
import router from '~/views/system/user/userBasicInfo/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#userBasicInfo')

  if (!el) {
    console.warn('#userBasicInfo not found')
    return
  }
