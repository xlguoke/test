...
import Main from '~/views/system/wisdom//environmentConfig/App/index.vue'
import router from '~/views/system/wisdom//environmentConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#environmentConfig')

  if (!el) {
    console.warn('#environmentConfig not found')
    return
  }
