...
import Main from '~/views/system/wisdom//videoMonitorConfig/App/index.vue'
import router from '~/views/system/wisdom//videoMonitorConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#videoMonitorConfig')

  if (!el) {
    console.warn('#videoMonitorConfig not found')
    return
  }
