...
import Main from '~/views/system/wisdom//connectDeviceConfig/App/index.vue'
import router from '~/views/system/wisdom//connectDeviceConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#connectDeviceConfig')

  if (!el) {
    console.warn('#connectDeviceConfig not found')
    return
  }
