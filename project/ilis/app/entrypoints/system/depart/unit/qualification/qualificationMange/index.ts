...
import Main from '~/views/system/depart/unit/qualification/qualificationMange/App/index.vue'
import router from '~/views/system/depart/unit/qualification/qualificationMange/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#qualificationMange')

  if (!el) {
    console.warn('#qualificationMange not found')
    return
  }
