...
import Main from '~/views/system/depart/unit/qualification/sealApplication/App.vue'
import router from '~/views/system/depart/unit/qualification/sealApplication/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#sealApplication')

  if (!el) {
    console.warn('#sealApplication not found')
    return
  }
