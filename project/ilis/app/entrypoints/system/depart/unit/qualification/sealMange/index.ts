...
import Main from '~/views/system/depart/unit/qualification/sealMange/App/index.vue'
import router from '~/views/system/depart/unit/qualification/sealMange/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#sealMange')

  if (!el) {
    console.warn('#sealMange not found')
    return
  }
