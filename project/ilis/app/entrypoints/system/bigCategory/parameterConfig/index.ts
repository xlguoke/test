...
import Main from '~/views/system/bigCategory/parameterConfig/App/index.vue'
import router from '~/views/system/bigCategory/parameterConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#parameterConfig')

  if (!el) {
    console.warn('#parameterConfig not found')
    return
  }
