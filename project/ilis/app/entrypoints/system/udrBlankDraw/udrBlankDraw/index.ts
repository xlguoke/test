...
import Main from '~/views/system/udrBlankDraw/udrBlankDraw/App/index.vue'
import router from '~/views/system/udrBlankDraw/udrBlankDraw/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#udrBlankDraw')

  if (!el) {
    console.warn('#udrBlankDraw not found')
    return
  }
