...
import Main from '~/views/system/icon/iconList/App/index.vue'
import router from '~/views/system/icon/iconList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#iconList')

  if (!el) {
    console.warn('#iconList not found')
    return
  }
