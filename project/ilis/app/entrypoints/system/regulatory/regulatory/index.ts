...
import Main from '~/views/system/regulatory/regulatory/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#regulatory')

  if (!el) {
    console.warn('#regulatory not found')
    return
  }
