...
import Main from '~/views/biddingManagement/biddingRecordDetail/App/index.vue'
import router from '~/views/biddingManagement/biddingRecordDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#biddingRecordDetail')

  if (!el) {
    console.warn('#biddingRecordDetail not found')
    return
  }
