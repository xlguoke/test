...
import Main from '~/views/biddingManagement/biddingPersonDetail/App/index.vue'
import router from '~/views/biddingManagement/biddingPersonDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#biddingPersonDetail')

  if (!el) {
    console.warn('#biddingPersonDetail not found')
    return
  }
