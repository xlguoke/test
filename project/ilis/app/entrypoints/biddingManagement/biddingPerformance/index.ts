...
import Main from '~/views/biddingManagement/biddingPerformance/App/index.vue'
import router from '~/views/biddingManagement/biddingPerformance/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#biddingPerformance')

  if (!el) {
    console.warn('#biddingPerformance not found')
    return
  }
