...
import Main from '~/views/biddingManagement/biddingRecord/App/index.vue'
import router from '~/views/biddingManagement/biddingRecord/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#biddingRecord')

  if (!el) {
    console.warn('#biddingRecord not found')
    return
  }
