...
import Main from '~/views/biddingManagement/biddingPerson/App/index.vue'
import router from '~/views/biddingManagement/biddingPerson/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#biddingPerson')

  if (!el) {
    console.warn('#biddingPerson not found')
    return
  }
