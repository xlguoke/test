...
import Main from '~/views/commonAuditProcess/relation/App/index.vue'
import router from '~/views/commonAuditProcess/relation/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#relation')

  if (!el) {
    console.warn('#relation not found')
    return
  }
