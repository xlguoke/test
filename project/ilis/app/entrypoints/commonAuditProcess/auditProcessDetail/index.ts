...
import Main from '~/views/commonAuditProcess/auditProcessDetail/App/index.vue'
import router from '~/views/commonAuditProcess/auditProcessDetail/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#auditProcessDetail')

  if (!el) {
    console.warn('#auditProcessDetail not found')
    return
  }
