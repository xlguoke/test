...
import Main from '~/views/commonAuditProcess/auditProcess/App/index.vue'
import router from '~/views/commonAuditProcess/auditProcess/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#auditProcess')

  if (!el) {
    console.warn('#auditProcess not found')
    return
  }
