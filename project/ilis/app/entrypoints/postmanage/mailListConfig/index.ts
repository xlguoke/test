...
import Main from '~/views/postmanage/mailListConfig/App/index.vue'
import router from '~/views/postmanage/mailListConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#mailListConfig')

  if (!el) {
    console.warn('#mailListConfig not found')
    return
  }
