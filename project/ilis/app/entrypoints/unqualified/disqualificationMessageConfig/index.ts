...
import Main from '~/views/unqualified/disqualificationMessageConfig/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#disqualificationMessageConfig')

  if (!el) {
    console.warn('#disqualificationMessageConfig not found')
    return
  }
  ...
  app.mount('#disqualificationMessageConfig')
})()
