...
import Main from '~/views/unqualified/queryMessage/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#queryMessage')

  if (!el) {
    console.warn('#queryMessage not found')
    return
  }
