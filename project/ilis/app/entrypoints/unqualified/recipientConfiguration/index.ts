...
import Main from '~/views/unqualified/recipientConfiguration/App/index.vue'
import router from '~/views/unqualified/recipientConfiguration/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#recipientConfiguration')

  if (!el) {
    console.warn('#recipientConfiguration not found')
    return
  }
  ...
  app.mount('#recipientConfiguration')
})()
