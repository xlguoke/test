...
import Main from '~/views/customtemplate/indicator/indicatorList/App/index.vue'
import router from '~/views/customtemplate/indicator/indicatorList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#indicatorList')

  if (!el) {
    console.warn('#indicatorList not found')
    return
  }
