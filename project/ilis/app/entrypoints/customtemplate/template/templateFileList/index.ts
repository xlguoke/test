...
import Main from '~/views/customtemplate/template/templateFileList/App/index.vue'
import router from '~/views/customtemplate/template/templateFileList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#templateFileList')

  if (!el) {
    console.warn('#templateFileList not found')
    return
  }
