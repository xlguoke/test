...
import Main from '~/views/customtemplate/template/templateFileSpecify/App/index.vue'
import router from '~/views/customtemplate/template/templateFileSpecify/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#templateFileSpecify')

  if (!el) {
    console.warn('#templateFileSpecify not found')
    return
  }
