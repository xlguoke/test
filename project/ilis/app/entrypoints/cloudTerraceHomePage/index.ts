...
import Main from '~/views/cloudTerraceHomePage/App/index.vue'
import router from '~/views/cloudTerraceHomePage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#cloudTerraceHomePage')

  if (!el) {
    console.warn('#cloudTerraceHomePage not found')
    return
  }
  ...
  app.mount('#cloudTerraceHomePage')
})()
