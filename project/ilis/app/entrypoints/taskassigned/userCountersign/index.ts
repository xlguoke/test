...
import Main from '~/views/taskassigned/userCountersign/App/index.vue'
import router from '~/views/taskassigned/userCountersign/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#userCountersign')

  if (!el) {
    console.warn('#userCountersign not found')
    return
  }
