...
import Main from '~/views/projectAuthority/auditAuthority/App/index.vue'
import router from '~/views/projectAuthority/auditAuthority/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#auditAuthority')

  if (!el) {
    console.warn('#auditAuthority not found')
    return
  }
