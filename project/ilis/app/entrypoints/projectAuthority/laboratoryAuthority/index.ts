...
import Main from '~/views/projectAuthority/laboratoryAuthority/App/index.vue'
import router from '~/views/projectAuthority/laboratoryAuthority/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#laboratoryAuthority')

  if (!el) {
    console.warn('#laboratoryAuthority not found')
    return
  }
