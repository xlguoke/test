...
import Main from '~/views/equipmentNew/consumables/consumablesComprehensiveList/App/index.vue'
import router from '~/views/equipmentNew/consumables/consumablesComprehensiveList/router/index'
import '~/main.ts'
import '~/providers/common/global.less'
import '~/providers/common/webTheme.less'
import '~/providers/common/styleConfig.less'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#consumablesComprehensiveList')

  if (!el) {
    console.warn('#consumablesComprehensiveList not found')
    return
  }
  ...
  app.mount('#consumablesComprehensiveList')
})()
