...
import Main from '~/views/equipmentNew/workbench/equipWorkbench/App/index.vue'
import router from '~/views/equipmentNew/workbench/equipWorkbench/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipWorkbench')

  if (!el) {
    console.warn('#equipWorkbench not found')
    return
  }
