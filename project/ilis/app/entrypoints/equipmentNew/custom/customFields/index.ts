...
import Main from '~/views/equipmentNew/custom/customFields/App/index.vue'
import router from '~/views/equipmentNew/custom/customFields/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#customFields')

  if (!el) {
    console.warn('#customFields not found')
    return
  }
