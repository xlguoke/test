...
import Main from '~/views/equipmentNew/check/checkPlanSubmit/App/index.vue'
import router from '~/views/equipmentNew/check/checkPlanSubmit/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#checkPlanSubmit')

  if (!el) {
    console.warn('#checkPlanSubmit not found')
    return
  }
