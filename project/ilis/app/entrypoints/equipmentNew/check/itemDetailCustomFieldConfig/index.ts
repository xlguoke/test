...
import Main from '~/views/equipmentNew/check/itemDetailCustomFieldConfig/App/index.vue'
import router from '~/views/equipmentNew/check/itemDetailCustomFieldConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#itemDetailCustomFieldConfig')

  if (!el) {
    console.warn('#itemDetailCustomFieldConfig not found')
    return
  }
  ...
  app.mount('#itemDetailCustomFieldConfig')
})()
