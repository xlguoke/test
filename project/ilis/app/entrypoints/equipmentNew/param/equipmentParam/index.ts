...
import Main from '~/views/equipmentNew/param/equipmentParam/App/index.vue'
import router from '~/views/equipmentNew/param/equipmentParam/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipmentParam')

  if (!el) {
    console.warn('#equipmentParam not found')
    return
  }
