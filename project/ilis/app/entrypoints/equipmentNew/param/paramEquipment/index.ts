...
import Main from '~/views/equipmentNew/param/paramEquipment/App/index.vue'
import router from '~/views/equipmentNew/param/paramEquipment/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#paramEquipment')

  if (!el) {
    console.warn('#paramEquipment not found')
    return
  }
