...
import Main from '~/views/equipmentNew/iotAuth/mspAuthRecord/App/index.vue'
import router from '~/views/equipmentNew/iotAuth/mspAuthRecord/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#mspAuthRecord')

  if (!el) {
    console.warn('#mspAuthRecord not found')
    return
  }
