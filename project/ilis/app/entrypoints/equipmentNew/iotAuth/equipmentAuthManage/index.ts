...
import Main from '~/views/equipmentNew/iotAuth/equipmentAuthManage/App/index.vue'
import router from '~/views/equipmentNew/iotAuth/equipmentAuthManage/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipmentAuthManage')

  if (!el) {
    console.warn('#equipmentAuthManage not found')
    return
  }
