...
import Main from '~/views/equipmentNew/iotAuth/equipmentAuthRecord/App/index.vue'
import router from '~/views/equipmentNew/iotAuth/equipmentAuthRecord/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipmentAuthRecord')

  if (!el) {
    console.warn('#equipmentAuthRecord not found')
    return
  }
