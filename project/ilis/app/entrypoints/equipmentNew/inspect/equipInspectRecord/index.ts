...
import Main from '~/views/equipmentNew/inspect/equipInspectRecord/App/index.vue'
import router from '~/views/equipmentNew/inspect/equipInspectRecord/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipInspectRecord')

  if (!el) {
    console.warn('#equipInspectRecord not found')
    return
  }
