...
import Main from '~/views/equipmentNew/egress/eqEgressList/App/index.vue'
import router from '~/views/equipmentNew/egress/eqEgressList/router/index'
import '~/main.ts'
import '~/providers/common/global.less'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#eqEgressList')

  if (!el) {
    console.warn('#eqEgressList not found')
    return
  }
