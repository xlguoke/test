...
import Main from '~/views/equipmentNew/equipment/equipmentUseRecord/App/index.vue'
import '~/main.ts'

import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#equipmentUseRecord')

  if (!el) {
    console.warn('#equipmentUseRecord not found')
    return
  }
