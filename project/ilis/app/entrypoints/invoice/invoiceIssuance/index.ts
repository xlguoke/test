...
import Main from '~/views/invoice/invoiceIssuance/App/index.vue'
import router from '~/views/invoice/invoiceIssuance/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#invoiceIssuance')

  if (!el) {
    console.warn('#invoiceIssuance not found')
    return
  }
