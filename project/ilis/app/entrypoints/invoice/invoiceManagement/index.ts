...
import Main from '~/views/invoice/invoiceManagement/App/index.vue'
import router from '~/views/invoice/invoiceManagement/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#invoiceManagement')

  if (!el) {
    console.warn('#invoiceManagement not found')
    return
  }
