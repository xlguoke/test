...
import Main from '~/views/reportManageNew/question/reportPrintStatistics/App/index.vue'
import router from '~/views/reportManageNew/question/reportPrintStatistics/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportPrintStatistics')

  if (!el) {
    console.warn('#reportPrintStatistics not found')
    return
  }
  ...
  app.mount('#reportPrintStatistics')
})()
