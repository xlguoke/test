...
import Main from '~/views/reportManageNew/question/reportGrantStatistics/App/index.vue'
import router from '~/views/reportManageNew/question/reportGrantStatistics/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportGrantStatistics')

  if (!el) {
    console.warn('#reportGrantStatistics not found')
    return
  }
  ...
  app.mount('#reportGrantStatistics')
})()
