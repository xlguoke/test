...
import Main from '~/views/reportManageNew/question/reportQuestionStat/App/index.vue'
import router from '~/views/reportManageNew/question/reportQuestionStat/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportQuestionStat')

  if (!el) {
    console.warn('#reportQuestionStat not found')
    return
  }
