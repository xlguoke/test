...
import Main from '~/views/reportManageNew/reportCommit/reportUploadSupervision/App/index.vue'
import router from '~/views/reportManageNew/reportCommit/reportUploadSupervision/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportUploadSupervision')

  if (!el) {
    console.warn('#reportUploadSupervision not found')
    return
  }
  ...
  app.mount('#reportUploadSupervision')
})()
