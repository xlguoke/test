...
import Main from '~/views/reportManageNew/integratedQuery/integratedQuery/App/index.vue'
import router from '~/views/reportManageNew/integratedQuery/integratedQuery/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#integratedQuery')

  if (!el) {
    console.warn('#integratedQuery not found')
    return
  }
