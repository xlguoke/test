...
import Main from '~/views/reportManageNew/allData/reportStandingBook/App/index.vue'
import router from '~/views/reportManageNew/allData/reportStandingBook/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportStandingBook')

  if (!el) {
    console.warn('#reportStandingBook not found')
    return
  }
