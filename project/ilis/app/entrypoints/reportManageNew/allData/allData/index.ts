...
import Main from '~/views/reportManageNew/allData/allData/App/index.vue'
import router from '~/views/reportManageNew/allData/allData/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#allData')

  if (!el) {
    console.warn('#allData not found')
    return
  }
