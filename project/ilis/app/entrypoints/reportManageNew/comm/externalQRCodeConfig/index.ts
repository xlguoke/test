...
import Main from '~/views/reportManageNew/comm/externalQRCodeConfig/App/index.vue'
import router from '~/views/reportManageNew/comm/externalQRCodeConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#externalQRCodeConfig')

  if (!el) {
    console.warn('#externalQRCodeConfig not found')
    return
  }
  ...
  app.mount('#externalQRCodeConfig')
})()
