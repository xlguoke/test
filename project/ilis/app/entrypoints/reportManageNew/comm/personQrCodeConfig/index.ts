...
import Main from '~/views/reportManageNew/comm/personQrCodeConfig/App/index.vue'
import router from '~/views/reportManageNew/comm/personQrCodeConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#personQrCodeConfig')

  if (!el) {
    console.warn('#personQrCodeConfig not found')
    return
  }
