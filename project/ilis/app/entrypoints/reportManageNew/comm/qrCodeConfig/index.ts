...
import Main from '~/views/reportManageNew/comm/qrCodeConfig/App/index.vue'
import router from '~/views/reportManageNew/comm/qrCodeConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#qrCodeConfig')

  if (!el) {
    console.warn('#qrCodeConfig not found')
    return
  }
