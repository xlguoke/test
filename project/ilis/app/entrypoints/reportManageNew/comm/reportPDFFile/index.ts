...
import Main from '~/views/reportManageNew/comm/reportPDFFile/App/index.vue'
import router from '~/views/reportManageNew/comm/reportPDFFile/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportPDFFile')

  if (!el) {
    console.warn('#reportPDFFile not found')
    return
  }

  const app = createApp(Main, {})

  await AppInitHelper.useStore(app)
  await AppInitHelper.useDirectives(app)
