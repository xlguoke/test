...
import Main from '~/views/reportManageNew/comm/reportQrCodeConfig/App/index.vue'
import router from '~/views/reportManageNew/comm/reportQrCodeConfig/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportQrCodeConfig')

  if (!el) {
    console.warn('#reportQrCodeConfig not found')
    return
  }
