...
import Main from '~/views/reportManageNew/copy/reportCopyList/App/index.vue'
import router from '~/views/reportManageNew/copy/reportCopyList/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#reportCopyList')

  if (!el) {
    console.warn('#reportCopyList not found')
    return
  }
