...
import Main from '~/views/inspection/keyPoint/keyPoint/App/index.vue'
import router from '~/views/inspection/keyPoint/keyPoint/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#keyPoint')

  if (!el) {
    console.warn('#keyPoint not found')
    return
  }
