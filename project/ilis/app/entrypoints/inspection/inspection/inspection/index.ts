...
import Main from '~/views/inspection/inspection/inspection/App/index.vue'
import router from '~/views/inspection/inspection/inspection/router/index'
import '~/main.ts'
import '~/providers/transformUtils/transform.less';

(async function () {
  const el = document.querySelector<HTMLElement>('#inspection')

  if (!el) {
    console.warn('#inspection not found')
    return
  }
