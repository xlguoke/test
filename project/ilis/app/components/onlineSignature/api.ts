import type { CONSIGN_SIGN_TYPE, REPORT_SIGN_TYPE } from '.'
import { IlisApiHelper } from '~/utils/IlisApiHelper'

// 签字人信息
export interface SignaturePerson {
  /** 身份 */
  identity: CONSIGN_SIGN_TYPE | REPORT_SIGN_TYPE
  /** 签字人姓名 */
  name: string
  /** 签字人手机号 */
  phone: string
 ...
 * 报告领取: 获取二维码路径
 */
export function reportQrcodeByPhone(phone: string, reportIds: string) {
  return IlisApiHelper.get<string>(`rest/report/issue/sign/qrcode?phone=${phone}&reportIds=${reportIds}`)
}

/**
 * # 报告领取：发送签名短信
 * @param reportIds 报告ID，多个逗号拼接
 * @param data 签名人手机号
 */
export function sendSignSmsReport(reportIds: string, data: string[]) {
  return IlisApiHelper.post(`rest/report/issue/sign/sms?reportIds=${reportIds}`, data)
}
