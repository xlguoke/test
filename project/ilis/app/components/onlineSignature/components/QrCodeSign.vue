    ...
    <div class="qrImg flex items-center">
      <div class="bg-[#e6f4ff] p-1 mr-4 w-[120px] h-[120px] rounded-[2px]">
        <div class="h-full bg-white flex justify-center items-center">
          <img v-if="qrCode" class="w-full h-full block" :src="qrCode" alt="">
          <p v-else class="text-gray-400 w-[60px]">
            选择人员后生成二维码
          </p>
        </div>
      </div>
...
async function getRrCodeUrl() {
  if (props.source === SIGNATURE_SOURCE.CONSIGN) {
    ...
    const { data } = await reportQrcodeByPhone(props.phone, props.dataId)
    qrCodelink.value = data
  }
}
