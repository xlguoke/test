<template>
  <div>
    ...
    <!-- 二维码签名 -->
    <QrCodeSign
      v-if="signType === SIGN_TYPE.QR_CODE"
      :source="source"
      :phone="selectedPhone"
      :data-id="dataId"
    />
...
import type { PropsData, SignatureData } from '.'
import type { SignaturePerson } from './api'
import { ExclamationCircleFilled } from '@ant-design/icons-vue'
import { message } from 'ant-design-vue'
import { useIndustryStore } from '~/store/industryStore'
import { CONSIGN_SIGN_TYPE, CONSIGN_SIGN_TYPE_DICT, ConsignUserEntity, OnlineUserEntity, REPORT_SIGN_TYPE, SIGN_TYPE, SIGN_TYPE_DICT, SIGNATURE_SOURCE, SPECIAL_SIGN_TYPE } from '.'
...
/** 是否为报告发放签字 */
const isIssueSign = computed(() => props.specialSignType === SPECIAL_SIGN_TYPE.ISSUE)
/** 签字类型 */
...
async function getSignUsers() {
  let res: any[] = []
  if (isIssueSign.value)
    res = initIssueSignUsers()
  else
    res = await getSignUsersApi()

  if (!res || res.length === 0)
    return

  if (!industryWitnessField?.selected) {
    res = res.filter(d => d.identity === CONSIGN_SIGN_TYPE.CONSIGN)
  }
  ...
  // 设置默认选中项
  if (res.length && signType.value === SIGN_TYPE.HAND_WRITE) {
...
/** 报告发放签字，初始化人员列表：只能批量同委托人的报告进行签字发放，故只显示其中一条数据的人员信息即可 */
function initIssueSignUsers() {
  ...
  return [{
    key: '',
    name: d.consignPerson,
    phone: d.consignPersonPhone,
    identity: REPORT_SIGN_TYPE.CURRENT,
    signed: !!d.receiverSignature,
  }]
}
