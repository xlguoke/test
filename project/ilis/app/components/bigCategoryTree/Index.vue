<template>
  <a-flex vertical class="h-full">
    <div class="pb-4">
      <div>检测参数版本：</div>
      <a-select v-model:value="paramVersionId" class="w-full" @change="onChangeVersion">
      ...
      <a-spin :spinning="treeLoading" tip="加载中...">
        ...
        >
          <template #switcherIcon="{ expanded }">
            <FolderOpenFilled v-if="expanded" class="text-colorPrimary text-lg" />
            <FolderFilled v-else class="text-colorPrimary text-lg" />
          </template>
        </a-tree>
      </a-spin>
    </div>
