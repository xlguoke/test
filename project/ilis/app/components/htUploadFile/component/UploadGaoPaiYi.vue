...
function downloadGaoPaiYi() {
  if (isDev) {
    window.open('http://192.168.2.65:8080/fileShare/setup/KMGpyServer_3.9.2.exe', '_blank')
  }
  else {
    window.open('https://pro.ilis.cn/fileShare/setup/KMGpyServer_3.9.2.exe', '_blank')
  }
}
...
function gpyUploadFile() {
  ...
  const validFileType = scanFiles.some((f: any) => !ACCEPT.value.includes(f.type))
  if (validFileType) {
    Modal.error({
      title: '文件格式错误，无法上传',
      content: `只支持${ACCEPT_STR.value}格式文件，请重新选择文件`,
      centered: true,
      okText: '确定',
    })
    return
