...
export async function upload(tag: string, path: string, proxy?: string) {
  ...
  if (UPLOAD_KEY)
    form.append('cidikey', UPLOAD_KEY)
  else
    console.log(colors.red('UPLOAD_KEY is not set, upload may fail'))

  const res = await fetch(
    `https://auth.qdm123.com/api/HitekFileUpload/Upload?FileType=FileType&FileTypeTag=${dir}`,
    {
      method: 'post',
      body: form,
      agent,
