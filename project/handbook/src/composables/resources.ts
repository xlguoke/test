...
export function useResourceFetch(type: basic.Data['type']) {
  ...
  async function procedure() {
    try {
      ...
      log.error(`查询数据类型为${type}的数据版本失败：`, e)
  ...
  function upgrade() {
    ...
    axios.post(
      ...
      if (result.success) {
        ...
        log.error('基础数据验证失败：', result.error)
  ...
  switch (resource.type) {
    ...
    case 'template':
      ...
      if (resource.hitekServer.endsWith('/UDR')) {
        log.warn('Hitek center URL before substring', resource.hitekServer)
        resource.hitekServer = resource.hitekServer.substring(0, resource.hitekServer.length - 4)
        log.warn('Hitek center URL after substring', resource.hitekServer)
      }
