...
const log = getLogger('useResource')
...
export function useResourceFetch(type: basic.Data['type'], auto?: boolean) {
  ...
  const version = ref('')
  const { setDataVersionStatus, getUpdateTime, updateInterval } = baseData()

  function resError(err: any) {
    const error = translateError(err)
    log.error('更新基础数据：', error)
    console.error('更新基础数据：', err)
  ...
  async function procedure() {
    try {
      ...
      if (!version.value || tenMinute(version.value)) {
        ...
        // 据上次更新不超过'检查更新间隔时间'时，不提示更新
        const t = getUpdateTime(type)
        if (!t || Date.now() - t > updateInterval)
