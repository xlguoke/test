...
export default defineStore('baseData', {
  state: () => {
    return {
      ...
      timer: null as any,
      // 记录离线数据最后更新时间
      updateTime: {} as { [key: string]: number },
      // 检查更新间隔时间
  ...
  actions: {
    setDataVersionStatus(type: string, status: string, msg: string) {
      ...
      if (status === 'finish')
        this.setUpdateTime(type)
    },
    setUpdateTime(type: string) {
      this.updateTime[type] = Date.now()
    },
    getUpdateTime(type: string) {
      return this.updateTime[type]
    },
  },
  persist: {
    paths: ['dataVersions', 'updateTime', 'autoUpdateNetwrok', 'autoUpdateTime'],
  },
})
