...
async function addParams() {
  ...
  if (!testItemId)
    ...
    return showToast({ type: 'fail', message: '请先选择样品' })

  addDefaultSelData(
    'SelParam',
    taskParams.value.map(d => ({
      ...d,
      id: d.testParamId,
      name: d.testParamName || '',
    })),
  )
...
watch(
  ...
  (list: Param[]) => {
    ...
    const newParam: TaskParam[] = []
    for (let i = 0; i < list.length; i++) {
      const item = list[i]
      if (addParam[item.id]) {
        newParam.push(addParam[item.id])
        continue
      }
      newParam.push({
...
<template>
  <div>
    ...
    <div class="add-param-btn">
      <van-button plain block @click="addParams">
        +添加参数
      </van-button>
    </div>
  </div>
