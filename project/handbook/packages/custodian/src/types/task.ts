...
const standard = z.object({
  ...
  baseStandardName: z.string().optional(),
...
const param = z.object({
  ...
  testParamName: z.string().nullable(),
...
const task = z.object({
  ...
  taskJsonRecordSets: z.array(jsonRecordset).nullable().optional(),
})

const taskData = task.omit({
  hitekServer: true,
  taskRecordSets: true,
  taskAppTemplates: true,
  taskParams: true,
  taskJsonRecordSets: true,
})
...
const taskDetail = z.object({
  taskData,
  taskRecordSets: z.array(recordset).min(1).nullable(),
  taskAppTemplates: z.array(template).min(1).nullable(),
  taskParams: z.array(param).min(1).nullable(),
  taskJsonData: z.object({
    note: z.array(jsonRecordset).nullable().optional(),
    log: z.array(jsonRecordset).nullable().optional(),
    jsonUdrData: z.array(jsonRecordset).nullable().optional(),
  }).nullable(),
  recordSetNameGroup: z.record(z.unknown()).nullable(),
  udrVBScripts: z.array(z.record(z.unknown())).nullable(),
  taskAttachment: z.array(attachment).nullable(),
})
...
export type JsonRecordset = z.infer<typeof jsonRecordset>
export type Template = z.infer<typeof template>
export const Validator = { task, taskDetail }
