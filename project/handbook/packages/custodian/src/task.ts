...
export async function saveTask(task: task.Task) {
  ...
  for (const rs of task.taskRecordSets) {
    tx.execute(
      'insert into task_recordset(taskId, testObjectId, testItemId, name, xml) values (?, ?, ?, ?, ?)',
      [taskId, rs.testObjectId, rs.testItemId, rs.name, rs.xml],
    )
  }
  task.taskJsonRecordSets?.forEach((set) => {
    tx.execute(
      'insert into task_json_recordset(taskId, testItemId, value) values(?, ?, ?)',
      [taskId, set.testItemId, set.value],
    )
  })
  tx.execute(
...
export async function saveOnsiteTask(task: SaveTask) {
  ...
  // delete persisted task params if present
  for (const persist of await findTaskParams(taskId))
...
export async function findTaskRecordSets(taskId: string) {
  return queryForList<task.Recordset>('select * from task_recordset where taskId = ?', [taskId])
}

export async function findTaskJsonRecordSets(taskId: string) {
  return queryForList<task.JsonRecordset>('select * from task_json_recordset where taskId = ?', [taskId])
}
...
function query2Tuple(query: TaskQuery): [conditions: string[], params: any[]] {
  ...
  if (query.createdFrom && !query.createdTo) {
    conditions.push('date(taskCreateTime) >= ?')
...
export async function getTaskTrace(taskId: string) {
  return (await queryForObject<{ trace: any }>('select * from task_trace where taskId = ?', [taskId])).trace
}

export async function getTaskHandwriting(taskId: string) {
  return (await queryForObject<{ handwriting: any }>('select * from task_handwriting where taskId = ?', [taskId])).handwriting
}
