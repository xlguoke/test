...
export function getAppLogFile(name: string): Promise<File> {
  return new Promise((resolve, reject) => {
    window.resolveLocalFileSystemURL(
...
export async function readLog(name: string): Promise<string> {
  const file = await getAppLogFile(name)
  return new Promise((resolve, reject) => {
...
export async function readLogStream() {
  const file = await getAppLogFile('app.log')
  // const stream = file.stream().pipeThrough(new CompressionStream('gzip'))
  return new Response(file).blob()
}

export async function readLogFileBuffer(): Promise<ArrayBuffer> {
  const file = await getAppLogFile('error.log')
  return new Promise((resolve, reject) => {
    const fileReader = new FileReader()
    fileReader.onload = () => {
