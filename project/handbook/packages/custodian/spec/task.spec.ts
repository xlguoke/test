...
import * as jsonRs from '../testdata/taskWithJson.json'
import taskTrace_ from '../testdata/udr-trace.json'
import udrHandwriting_ from '../testdata/udr-handwriting.json'
...
describe('task management', () => {
  ...
  it('should save task with json record set', async () => {
    const data = task.Validator.task.parse(jsonRs)
    const taskId = await saveTask(data)
    const recordSets = await findTaskJsonRecordSets(taskId)
    expect(recordSets.length).toBe(1)
    expect(recordSets[0].testItemId).toBe('8bb413fd-5453-4ff3-9fc1-c9e2e82dcd3f')
    expect(recordSets[0].value).not.toBeNull()
  })
  ...
  it('can save onsite-task again', async () => {
    const recordSets = gen(genRecordset)
    await saveRecordset(recordSets)
    const saveTask = genSaveTask()
    const taskId = await saveOnsiteTask(saveTask)
    saveTask.id = taskId
    await saveOnsiteTask(saveTask)
    const params = await findTaskParams(taskId)
    expect(params.length).toBe(5)
  })
  ...
  it('should scroll tasks', async () => {
    ...
    query = new TaskQuery()
    const params = await findTaskParams(findings[0].id)
    query.params = getRandomItem(params)!.testParamDisplayName
    findings = await findTasks(query)
    expect(findings.length).withContext('finding with params').toBeGreaterThanOrEqual(1)
    query.createdTo = formatDate(findings[0].taskCreateTime)
    findings = await findTasks(query)
    expect(findings.length).withContext('finding with createTo').toBeGreaterThanOrEqual(1)
  }, 10000)
  ...
  it('should retrieve the saved task trace', async () => {
    await executeSql(`insert into task_trace(taskId, trace) VALUES (?, ?)`, ['one', JSON.stringify(taskTrace_)])

    const trace = await getTaskTrace('one')
    expect(trace).toBe(JSON.stringify(taskTrace_))
  })

  it('should retrieve the saved task handwriting', async () => {
    await executeSql(`insert into task_handwriting(taskId, handwriting) VALUES (?, ?)`, ['one', JSON.stringify(udrHandwriting_)])

    const handwriting = await getTaskHandwriting('one')
    expect(handwriting).toBe(JSON.stringify(udrHandwriting_))
  })
})
