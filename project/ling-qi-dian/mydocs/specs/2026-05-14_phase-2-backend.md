...
- In:
  ...
  - **云函数开发（30 个，按模块分组）**：
    ...
    **会员模块（5 个）**
    | `member-info` | 会员信息 | 等级/积分/余额/消费/等级名称/权益说明 |
    | `member-benefits` | 等级福利配置 | 获取当前等级权益 + 所有等级对比表 |
    | `member-point-logs` | 积分流水 | 分页、按类型筛选 |
    | `member-level-check` | 等级更新 | 根据累计充值金额检查等级（内部调用）|
    | `member-level-progress` | 等级进度 | 返回升级进度 |

    **会员充值模块（4 个）**
    | `member-recharge-tiers` | 充值档位列表 | 获取5个档位(¥198/¥680/¥1,000/¥3,000/¥8,000)、计算赠送金额/余额/积分 |
    | `member-recharge` | 发起充值 | 校验档位→创建充值订单→调用收钱吧→返回jsapi参数 |
    | `member-recharge-callback` | 充值回调 | 收钱吧验签→更新余额→发放余额和积分→检查等级→记录流水 |
