...
- In:
  ...
  - **uni-app 点单端（`packages/app/src/pages/`）**：10 个页面（全平台通用）
    ...
    | `/member/index` | 会员中心 | 头像/昵称/等级/积分/余额/累计消费、会员码、福利入口、充值入口、升级进度 |
    | `/member/benefits` | 会员福利 | 等级权益说明、升级规则、等级对比表（6级：普通用户/青铜/白银/黄金/铂金/钻石/至尊）、机车特色权益（洗车/保养/救援/备用车） |
    | `/member/recharge` | 会员充值 | 5个档位选择（¥198/¥680/¥1,000/¥3,000/¥8,000）、赠送金额/余额/积分展示 |
    | `/member/rechargeLogs` | 充值记录 | 分页充值历史列表 |
    | `/member/pointLogs` | 积分明细 | 分页流水列表、按类型筛选 |
...
### 第 3 步：会员模块
- 会员中心 + 积分流水 + 会员码展示页
- 会员福利页（等级权益说明、升级规则、等级对比表）
- 会员充值页（常规档位 + 直升档位选择）
- 充值记录页
- Mock 会员数据（覆盖 6 个等级、5 个充值档位）
