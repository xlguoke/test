...
> ⚠️ 本阶段是整个项目的「契约层」，产出 4 份设计文档。
> 前端基于 API Contacts 可直接 Mock 跑通全部流程（不依赖后端）；
> 后端基于 DB Schema + API Contracts 逐接口实现（不依赖前端）。

## Goal
- 要解决的问题：前后端没有统一的接口契约和数据结构定义，无法并行开发
- 验收结果：4 份设计文档完整覆盖系统全部功能，前/后端拿到即可独立开工

## Done Contract
- 完成定义：
  - `mydocs/db-schema.md`：全部 9 个集合的字段定义、类型、索引、关联关系
  - `mydocs/api-contracts.md`：全部 34 个云函数的接口契约（路径、Method、Req/Res 类型、错误码）
  - `mydocs/member-system-design.md`：会员等级、充值、积分完整设计
  - `mydocs/frontend-routes.md`：三端全部页面路由、组件树、数据流
  - `packages/shared/src/types/`：与 API Contracts 对应的 TypeScript 类型文件
- 证明来源：review 四份文档，覆盖方案文档中所有功能点，无一遗漏
...
- In:
  ...
  - **前端路由文档（`mydocs/frontend-routes.md`）**：
    ...
    - 后台管理端：9 个页面路由 + 菜单结构
    - 三端共享组件清单
  - **会员体系设计文档（`mydocs/member-system-design.md`）**：
    - 会员等级体系（6级定义：普通用户/青铜/白银/黄金/铂金/钻石/至尊）
    - 各等级权益明细（折扣率、积分倍率、生日礼、机车特色权益：洗车/保养/救援/备用车）
    - 升级规则（累计充值金额门槛：¥0/¥1,000/¥3,000/¥8,000/¥15,000/¥30,000）
    - 等级永久保留，无降级，无续期要求
    - 会员充值规则（平时5档：¥198/¥680/¥1,000/¥3,000/¥8,000，开业活动4档：¥680/¥1,000/¥3,000/¥8,000，统一赠送余额10%，同时赠送余额和积分）
    - 累计充值金额与积分规则（等级根据累计充值金额确定，无过期；积分用于抵扣，12个月有效期，余额消费不给积分）
    - 相关数据库集合补充（recharge_logs、member_benefits）
    - 相关API接口补充（member-recharge、member-benefits等）
  - **开业活动计划文档（`mydocs/opening-activity-plan.md`）**：
    - 开业活动为独立模块，主体程序开发完成后再添加
    - 活动规则（充值赠送积分1.5倍，限时30天）
    - 技术实现（活动配置表、云函数、前端页面）
    - 开发排期（约5.5天，主体程序上线后开发）
