    ...
    <div class="person-statistics">
      ...
      <div class="person-statistics-right">
        <div class="person-statistics-bar one">
          <span class="person-statistics-title">持证人数</span>
          <span class="person-statistics-count">
            {{ data?.["持证人数"] }}
          </span>
        </div>
        <div class="person-statistics-bar two">
          <span class="person-statistics-title">正高工数</span>
          <span class="person-statistics-count">
            {{ data?.["正高工数"] }}
          </span>
        </div>
...
const getData = () => {
  personStatistics({ type: 1 }).then((res) => {
    if (res.code === 20000) {
      const obj = {}
      res.data.forEach((d) => {
      ...
      numAnimate(data.value, obj, (data) => {
        data.value = {
          ...
          持证人数: numberToCurrencyNo(data.持证人数),
          正高工数: numberToCurrencyNo(data.正高工数),
