  ...
  <FrameBox class="flex-1">
    ...
    <div class="h-full flex flex-col">
      <div class="text-center pt-30 text-[0.36rem]">
        <span class="text[#E6FFFC]">本{{ period }}收样组数：</span>
...
const init = useAutoRequestHooks({
  ...
  api: (q) =>
    request({
      url: `api/dashboard/sgjc/sample-acceptance/sample/trend/filter?range=${q}`,
      method: "get"
    }),
  ...
  setData: (res) => {
    ...
    options.yAxis.name = "单位：（组）"
    options.xAxis.data = data.map((i: any) => i.name)
    options.series[0].name = "收样数量"
