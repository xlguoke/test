<template>
  <FrameBox>
    <template #title>
      <FrameTitle>资质证书</FrameTitle>
    </template>

    <div class="pt-60 h-full certificate-wrap">
      <swiper
        class="h-full"
        :slides-per-view="4"
      ...
      >
        <swiper-slide v-for="(item, index) in data" :key="index">
          <div class="certificate-img" @click="onPreview(item)">
            <img :src="item.url" />
          </div>
          <p class="certificate-title">{{ item.name }}</p>
        </swiper-slide>
      </swiper>
    </div>
...
const data = computed(() => {
  const list = companyData.value.filter((i) => i.type === CompanyInfoType.资质)
