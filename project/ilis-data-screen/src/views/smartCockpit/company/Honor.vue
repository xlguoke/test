<template>
  <FrameBox>
    <template #title>
      <FrameTitle>荣誉奖励</FrameTitle>
    </template>

    <div class="pt-120 h-full honor-wrap">
      <swiper
        class="h-full"
        style="padding-bottom: 1rem"
        :slides-per-view="3"
        :space-between="clacChartPx(30)"
      ...
      >
        <swiper-slide v-for="(item, index) in data" :key="index">
          <div class="honor-img" @click="onPreview(item)">
            <img :src="item.url" />
          </div>
          <p class="honor-title">{{ item.name }}</p>
        </swiper-slide>
      </swiper>
    </div>
...
const data = computed(() => {
  const list = companyData.value.filter((i) => i.type === CompanyInfoType.荣誉奖励)
