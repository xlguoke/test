<template>
  <CommonModal ref="pop" modal-title="报告无监控视频记录">
    <template #function>
      <div class="flex justify-end">
        <DateFilterSplit
          date-filter-title="试验提交时间"
...
import { FilterDateType, getLackVideoReportPage } from "@/api/smartCockpit.test.api"
...
const columns = ref<DataTableHead[]>([
  ...
  { title: "试验提交时间", key: "submit_date", width: "6rem" },
  { title: "报告编号", key: "report_number", width: "6rem" },
  { title: "任务编号", key: "test_task_code", width: "6rem" },
  { title: "样品名称", key: "test_sample_display_name", width: "6rem" },
  { title: "试验参数", key: "test_param_display_name", align: "left" }
])
...
// 初始化列表数据
async function getLackVideoReportPageData() {
  ...
  try {
    loading.value = true
    const { data, totalNum } = await getLackVideoReportPage(newParam).finally(() => {
      loading.value = false
    })
