...
import { filesType } from "@/type/common"
const bucket = "pub" // 公共桶
export const reportBucket = "reports" // 报告文件存储桶
// const isDev = import.meta.env.VITE_NODE_ENV === "development"

const minioPutObject = (file: filesType, isRandomName = false, bucketUri: string): Promise<any> => {
  ...
  return new Promise((resolve, reject) => {
    http({
      ...
      console.log("🚀 ~ file: minio.config:22 ~ res", res)
      if (!res.etag) {
        return reject(res)
      }
      return resolve({
