<template>
  <div
    class="file-list-box"
    :style="`display:${
      listType === 'picture-card' ? 'inline-block;float:left;padding-top:0;' : 'block'
    }`"
    @click.stop="() => {}"
  >
    <ul v-if="listType === 'text'" class="upload-text-list">
      <li v-for="(file, i) in fileDatas" :key="i" class="file-item">
        <span class="file-name font-1" @click="showFile(file)">
          {{ file.name }}
          <loading-outlined v-if="file.loading" />
        </span>
        <a-button type="link" danger size="small" @click="removeFile(i)">
          <delete-outlined />
        </a-button>
      </li>
    </ul>
    <ul v-else-if="listType === 'picture-card'" class="upload-card-list clearfix">
      <li v-for="(file, i) in fileDatas" :key="i" class="file-item">
        ...
        <p class="preview-box">
          ...
          <delete-outlined @click="removeFile(i)" />
        </p>
      </li>
    </ul>
    <ul v-else class="show-file-list clearfix">
      <li v-for="(file, i) in fileDatas" :key="i" class="file-item" @click="showFile(file)">
        ...
        <a-button type="link" danger size="small" @click.stop="removeFile(i)">
          <delete-outlined />
        </a-button>
      </li>
    </ul>
