...
const emits = defineEmits<{
  ...
  (e: 'confirm', value: IProjectOrg[]): void
  (e: 'cancel'): void
}>()
...
function customRow(record: IProjectOrg) {
  return {
    onClick: () => {
      if (record.disabled && record.children && record.children.length > 0) {
...
const filterData = computed(() => {
  let filterData: IProjectOrg[] = []
  if (!formState.value.quickQry) {
    filterData = dataSource.value
  }
  else {
    const searchItem = (item: IProjectOrg) => {
