...
export function useGpyCallback(config: IGpyConfig) {
  ...
  function GetResolutionResultCB(resCount: number, resArr: any[]) {
    ...
    form.resolution = Math.ceil(_resolutionArr.length / 3)
    nextTick(() => {
      toSetResolution()
    })
  }
   ...
   */
  async function AddImgFileToPDFResultCB(flag: number, _base64Str: string) {
    if (flag !== 0) {
    ...
    if (form.customName) {
      const typeItem = fileTypeDatas.value.find(d => d.value === form.fileType)
      fileName = await AnyDialogHelper.showModel(CustomName, { name: fileName, fileType: typeItem?.title })
    }
    tempCombinePath.value = `${fileFolder.value + fileName}.pdf`
