...
import { MenuList, RoleList, SetUsers } from './roles/components'
import type { DataSource as UserData } from '@/types/users.d'
import type { BaseItem, DataSource, EmitData, RoleUser } from '@/types/roles.d'
import useMyFetch from '@/composables/useMyFetch'

// 权限菜单
const type = ref('')
const roleInfo = ref<BaseItem>({
  id: '',
  name: '',
})
const api = ref(``)
...
function setPermissions(data: EmitData) {
  ...
  api.value = `/api/v1/roles/${roleInfo.value.id}`
  execute()
}

// 删除角色下用户
async function delRoleUser(obj: RoleUser) {
  const { error } = await useMyFetch(`/api/v1/roles/${roleInfo.value.id}/user`, {
    method: 'DELETE',
    body: JSON.stringify({
...
async function saveRoleAuth(keys: string[], cb: (bol: boolean) => void) {
  ...
  const { error } = await useMyFetch(`/api/v1/roles/${roleInfo.value.id}/permission`, {
    method: 'PUT',
    body: JSON.stringify({
      permissions,
    }),
  ...
  <div class="flex h-full">
    ...
    <div class="w-2/5 h-full">
      ...
      >
        ...
        >
          <div py-2>
            <span font-600 mr-3>{{ type === 'user' ? '用户' : '权限' }}列表</span>
            <span>当前设置角色：{{ roleInfo?.name }}</span>
          </div>
        </div>
        <!-- 用户列表 -->
        <SetUsers
